...
- **Empty state** (`TracesEmptyState`): a three-step setup guide in the Sentry mould, shown until this agent's first trace arrives. **One step at a time**: a later step is not rendered at all until the one before it is finished, and a finished step collapses to its heading (click to reopen; opening one closes the others). `reached` tracks the furthest step, `openStep` what is expanded. Each card copies the Connection check card (`border border-border rounded-xl p-3 md:p-4 space-y-3`).
  ...
  2. **Send one request per turn** — a two-column split (same `md:grid md:grid-cols-2` as `AgentConnectionTabContent`): left is a language segmented control (cURL / Python / JavaScript / Go, `src/components/traces/ingestSnippets.ts`, styled like the one in `AddRunToLabellingTaskDialog`) over a `CodeBlock` with the copy button; right is the `SNIPPET_FIELDS` table explaining `POST /traces`, `X-API-Key`, `agent_id`, `message_id`, `conversation_id`, `input`, `output`. Every snippet is pre-filled with the resolved backend URL, this agent's uuid, and the key from step 1 (`sk_...` until one is made). "I have added this" advances to step 3.
...
- **Code highlighting** (`src/components/traces/CodeBlock.tsx`): a ~15-line regex that colours strings and comments only. Deliberately not a highlighter dependency — four fixed snippets don't justify one.
