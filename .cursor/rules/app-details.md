...
- **Trace scoring.** The **Score new traces automatically** switch lives in a Traces section on the agent **Settings** tab, saved at once through `PUT /agents/{uuid}` with only `auto_score_traces` (on by default once the backend change lands). Which linked evaluators can score comes from JWT-only `GET /agents/{uuid}/trace-scoring-eligibility`; turning the switch on is refused with a 422 when none can. The ones that cannot are named with their reason (`wrong_type_for_agent`, `no_live_version`, `declares_variables`, in ordinary words). `useAgentTraceScoring` is called once in `AgentDetail` and handed to both tabs as `TraceScoringControls`. Once traces exist, the Traces tab shows one line saying scoring is on with a **Turn off** button (or that it is off, with a Turn on in Settings button) and a second line naming the evaluators that cannot score traces and why. When any trace on the page carries `latest_run_error: "over_limit"`, a third line says the workspace has scored as many traces as its limit allows, with the same contact link the limit toast uses (`CONTACT_LINK`). The row's Failed or Skipped pill names the reason on hover, through `scoringRunErrorCopy`.
