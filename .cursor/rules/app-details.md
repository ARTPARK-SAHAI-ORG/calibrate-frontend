...
- **Data model** (`src/app/page.tsx`): Each entry is **`{ id, label, sections[] }`** where **`sections`** is **`LandingFeatureSection[]`**: **`{ headingBold, headingLight, description, images: string[] }`** plus optional **`quickStart?: { steps; closingHeadline?; comparisonSteps? }`** where **`steps`** / **`comparisonSteps`** (if present) are **`LandingQuickStartStep[]`** and **`LandingQuickStartStep`** is **`{ key, headingBold, headingLight, image }`** (module-local type alias). There is **no** **`quickStart.heading`** — tab copy uses **`section.headingBold`**, **`headingLight`**, **`description`**. **`Text agents`** section intro: *Evaluate the quality* / *of your AI responses* (centered **`h2`**). **`quickStart.comparisonSteps`** has **field-level JSDoc** (**second ladder** after **`closingHeadline`; **`QuickStartStepIndicator` `total`** = **`comparisonSteps.length`**, numbering **1…n independent of **`steps`**). **`closingHeadline`** (**field JSDoc** on **`closingHeadline`**) is optional; **`comparisonSteps`** can exist **without** it—render order is **`steps`** → **`closingHeadline`** (if set) → **`comparisonSteps`** (if nonempty). **`Text agents`** today chains **three **`steps`** rasters**, *Find the best LLM* banner, **`llm-multi-input.png`** → **`llm-multi-output.png`** **comparison ladder** (**bench / leaderboard UI** marketing—canonical strings in **`tabs`**). **`images[]`** duplicates **five** **`public/`** paths in **primary ladder order then **`comparisonSteps` order**. **`runtime`** previews use **`steps` / **`comparisonSteps` `image`** only—not flat **`images[]`**. **`key`** anchors React **`key`** and **`alt`** substrings. **`Superseded`** for **historical** **`tabs`**: *Choose the best LLM* **as the sole section intro**—today that line is the **`closingHeadline`** banner, not the **`h2`** intro. Tabs **without **`quickStart`** use **`LandingFeatureImageColumn`**. **`Voice`** uses **two **`sections`** objects** today; **`llm`**/**`simulations`** use **one **`sections`** item** each.
- **Heading hierarchy — `quickStart` (Text agents)**: The **first** **`h2`** is the **section** value prop (**`headingBold` / `headingLight`**) plus **`description`** (**`mx-auto max-w-2xl text-center`**). Then **`steps`**: **`QuickStartStepIndicator`** + **`h3`** per row. Optional **`closingHeadline`**: **`h2`** only, centered. Optional **`comparisonSteps`**: repeats indicator + **`h3`** with **`total`** = **`comparisonSteps.length`** (shows **Step 1/2**, **Step 2/2**, not continuation of **`steps`**). Indicator styling (**emerald** pill, **`aria-label`**, fraction chip **`aria-hidden`**) matches **`steps`**. **`h3`** uses **`featureHeadlineClass`**.
- **Rendering pattern** — **within** the selected tab:
  - **Shared step stacks** (**module-local** **`page.tsx`**): **`LandingQuickStartDesktopStepStack`** takes **`steps`**, **`secIdx`**, **`activeSectionsLen`**, **`tabLabel`**, **`featureHeadlineClass`**—wraps **`flex flex-col gap-14 md:gap-16 lg:gap-20`**, **`lg:grid-cols-[400px_1fr]`** rows, **`QuickStartStepIndicator` `total` = `steps.length`**. **`LandingQuickStartMobileStepStack`** takes **`steps`**, **`secIdx`**, **`tabLabel: string`**, **`featureHeadlineClass`**—**`gap-12`**, **per-step **`flex flex-col gap-4`**. **Why**: **one** implementation for **primary **`steps`** and **`comparisonSteps`** so layout and chip math stay aligned. **Desktop `alt`**: if **`activeSectionsLen > 1`**, pattern **`${tabLabel} ${key} preview ${secIdx + 1}-${idx + 1}`**; else **`Feature ${key} preview ${idx + 1}`**. **Mobile `alt`**: always **`${tab.label} ${key} preview ${secIdx + 1}-${idx + 1}`** (even when only one **section** is showing in the **tabbed** desktop view—**`md:hidden`** shows **all** tabs).
  - **Desktop** (`md+`): **`desktopSectionRow`** → **`max-w-7xl mx-auto`**: centered **intro** (**`h2`** + **`p` `text-sm md:text-base`**), **`LandingQuickStartDesktopStepStack`** (**`quickStart.steps`**), optional **`closingHeadline`**, optional **`LandingQuickStartDesktopStepStack`** (**`comparisonSteps`**, wrapper **`mt-14 md:mt-16 lg:mt-20`**). Without **`quickStart`**: **`LandingFeatureImageColumn`**. **Voice `sections.length > 1`**: unchanged stacked **`desktopSectionRow`** **layout**.
  - **Mobile** (`below md`): **`quickStart`**: **intro** + **`LandingQuickStartMobileStepStack`** (**`steps`**) + **`closingHeadline`** + optional **`comparisonSteps`** stack (same **top margin** as **desktop**); else **`LandingFeatureImageColumn`**. Whole-tab **`space-y-12`** unchanged.
...
- **Gotcha — comparison ladder & helpers**: **`LandingQuickStartDesktopStepStack`** / **`LandingQuickStartMobileStepStack`** are the **single** place to edit **grid / gap / chip** behavior for **both** ladders; **`desktopSectionRow`** only **passes **`steps`** vs **`comparisonSteps`**. **`QuickStartStepIndicator` `total`** = **the array being rendered** (never **`steps.length`** while mapping **`comparisonSteps`**). If a documentation or a11y standard demands **fewer **`h2`** elements**, lower semantic rank on banners while keeping **`featureHeadlineClass`**.
...
- **Desktop content panel** (`md+`): **`quickStart`** **Text agents**: **intro** → **`steps`** grids → **`closingHeadline`** → **`comparisonSteps`** (when set)—see **`LandingQuickStartDesktopStepStack`**. Without **`quickStart`**: **`LandingFeatureImageColumn`** / Voice stacking unchanged.
- **Mobile layout** (`below md`): **`quickStart`** chain matches **desktop** ( **`LandingQuickStartMobileStepStack`** ); **`comparisonSteps`** inherits same margins.
...
- **Gotcha — image stack order**: Non-**`quickStart`**: **`images[]`** via **`LandingFeatureImageColumn`**—**no carousel**. **Text agents (`quickStart`)**: previews from **`steps[].image`** **and**, when set, **`comparisonSteps[].image`** (same **`grid`** / **`items-center`** as primary ladder). On narrow viewports **text stacks above** image. **Vertical** spacing between rows **`gap-14 md:gap-16 lg:gap-20`**. **`alt`**: keyed by **`step.key`**. **`featureHeadlineClass`** must stay aligned across **`h2`** / **`h3`**. Re-check **`QuickStartStepIndicator`** when **`total`** (**per** **`steps`** or **`comparisonSteps`**) is **1** or **>4**. Tweak **emerald** tokens with **landing** accents—avoid stray **hex** outside existing patterns.
