...
│   │   ├── page.tsx           # Landing page (marketing: responsive hero — shorter min-height band on mobile; `md+` near-full-viewport hero; open-source pill; institutional eyebrow; desktop sticky product-area nav + scroll spy; below `md` each product tab renders as in-flow nav button then section; integrations, community, etc.)
...
2. **Hero section** (responsive viewport band): Outer shell **`relative flex flex-col justify-center overflow-hidden`** — **vertical centering on all breakpoints** so headline block sits mid-band. **Mobile**: **`min-h-[56dvh]`** (shorter than a full phone viewport—reduces empty space above/below the hero copy; tune if marketing wants a tighter or taller band). **`md+`**: **`md:min-h-[calc(100dvh-5.25rem)]`** for a **near–first-screen** hero under **`LandingHeader`** (**`5.25rem`** ≈ header + border—retune if **`LandingHeader`** height changes). **Decorative** grid + circles: **`absolute`**, **`pointer-events-none`**. **Inner** stack **`max-w-4xl mx-auto`**, **`px-4 py-8 md:px-8 md:py-20 lg:py-24`**, **`text-center`**: **open-source pill** (**`mb-4 md:mb-6`**) → **`<h1>`** (**`mb-4 md:mb-7`**, **“AI evaluation platform”** / **`for non-profits`**, `<br />`) → **subtitle** → **institutional eyebrow** with **`mt-5 md:mt-14`** (tighter subtitle→eyebrow gap on mobile). **Why split behavior**: **`md+`** keeps **value prop + attribution** in one **tall** band aligned to “first screen”; **phone** uses a **deliberately shorter** min-height + less inner **`py-*`** so users reach feature content sooner—different tradeoff than **content-only** height with **no** **`min-h`**. **`document.title`** (*for NGOs*) may still differ from visible **`h1`**—align if marketing standardizes. **Obsolete**: **no** mobile **`min-h`** at all; hero inner **`py-14`** on mobile (replaced by **`py-8`**); **institutional** **`mt-8 md:mt-14`** when **`mt-5`** is used on mobile; **`justify-start`** / **`md:justify-center`** only (centering is now base **`justify-center`**).
3. **Feature areas (marketing blocks)**: **`max-w-7xl mx-auto`** wraps a **desktop-only** sticky **product-area** **`<nav>`** (**`hidden md:flex`**) plus a **single column** of **tab groups**. **Below `md`**: the **`<nav>`** is **not** shown; instead **each** of **Text agents** / **Voice agents** / **Simulations** is **`renderLandingProductNavButton` → `<section id={landing-…}>`** (button stacked above that tab’s content, **`md:hidden`** on the button wrapper). **From `md:`**: the **horizontal** sticky nav row appears **first**, then the three **`<section>`**s in order (sections are **not** nested inside the nav). **No** institutional `<p>` here (it lives in the hero). The outer wrapper is **`px-6 md:px-8 lg:px-12 pt-8 md:pt-12 lg:pt-14 pb-20 md:pb-20`** — **roomier** **`pt-*` / `pb-*` on mobile** so the product-area blocks do not feel glued to the hero or the procurement band.**Why**: marketing wanted less **dense** vertical rhythm on phones while keeping **`md+`** unchanged. The **nav strip** has **no** bordered/rounded/frosted “chrome” frame—only the **per-tab** controls are **card** buttons. **Sticky** (**desktop nav only**): **`sticky top-0 z-40`** (not **`lg:top-24`** — **`LandingHeader`** is **not** fixed). **`bg-white`** + **`z-40`** keep the strip above in-flow section content. **Scroll spy** — see **Feature areas (stacked sections + nav + scroll spy)** for **`measureActiveSection`**. **Click** still uses **`scrollIntoView`** and explicit state. **Main column** does **not** repeat the category title or `navDescription` **inside** each **`<section>`**—that copy lives **only** in the nav **cards** (desktop) or the **in-flow** card **above** each section (mobile). In-product **LLM Tests** naming on **`/tests`** is unchanged. **Obsolete**: **`pt-6 … pb-16`** on this feature shell alone (superseded by **`pt-8 … pb-20`** for the same block).
...
**Open source pill (hero)** (`src/app/page.tsx`, inside the hero **`max-w-4xl`** stack): A **`div`** with **`flex justify-center`** and **`mb-4 md:mb-6`** wraps the pill **above** the **`<h1>`**. Same pill tokens: `rounded-md border border-emerald-200/90 bg-emerald-50/90`, **`uppercase`** + **`tracking-wider`** on **`font-semibold text-emerald-950`**, `text-[10px] md:text-[11px]`, `px-1.5 py-0.5`, `shadow-[0_1px_0_rgba(0,0,0,0.04)]`. JSX copy is **Open source** (renders as **OPEN SOURCE** via **`uppercase`**). **Obsolete**: **`mb-5 md:mb-6`** from the pre–mobile-tightening pass; appending this pill **after** the GoK segment in the **institutional** line.

**Institutional eyebrow** (`src/app/page.tsx`, the **BUILT BY / FUNDED BY** line): Rendered **inside the hero** stack (**same `max-w-4xl` column** as the **`h1`**) **after** the subtitle, with **`mt-5 md:mt-14`**—tighter **subtitle → eyebrow** gap on **mobile** than on **`md+`**. **Obsolete**: **first child** of **`max-w-7xl`** before **`<nav>`**; **`mt-8 md:mt-14`** / **`mt-12 md:mt-14`** from earlier passes.

- **Why**: Keep **Artpark** outbound link, **IISc**, **Karnataka** funding, and **quiet lead-ins** with the hero message—on **`md+`** they share one **centered** first-screen band; on **mobile** the same copy sits in a **shorter** hero band (**`min-h-[56dvh]`**, **`mt-5`** after subtitle) so users reach feature content faster. **Typography hierarchy**: **BUILT BY** / **FUNDED BY** as **quiet labels** (`text-gray-400 font-normal`); **ARTPARK** (link), **`@ IISc`**, **GOVERNMENT OF KARNATAKA** (**semibold**) as **scannable** anchors. The **open-source** signal lives in the **hero pill** above the **`<h1>`**, not on this row.

- **What**: Single `<p>` with structure **BUILT BY** in a muted **`<span className="text-gray-400 font-normal">`** (same as **FUNDED BY**) → **linked ARTPARK block** (`<a>` …): **`<span>ARTPARK</span>`** then **`/artpark-mark.webp`** `<img>` — then **`@ IISc ·`** → **`FUNDED BY`** in **`<span className="text-gray-400 font-normal">`** → **`GOVERNMENT OF KARNATAKA`** in **`font-semibold text-gray-800 tracking-wide`**. The mark stays **decorative** `alt=""`. **No** open-source pill on this row.

- **Styling**: Eyebrow `<p>` uses **`text-center`**, `text-[11px] md:text-[13px]`, **`font-medium`**, **`text-gray-500`**, `tracking-wide`, `text-balance`, `leading-snug`, `max-w-2xl mx-auto` (no **`mb-*`**—vertical handoff to the **feature** wrapper is **`hero`** inner **`py-*`** plus **`pt-8 md:pt-12 lg:pt-14`** on the **feature** band; on **`md+`** the next major landmark is the **sticky** product **`nav`**, on **mobile** the **first** **`renderLandingProductNavButton`** for **Text agents**). **Quiet labels** override with **`text-gray-400 font-normal`**. ARTPARK `<a>`: **`inline`**, `text-inherit`, underline/decoration, hover to darker gray (`cursor-pointer`, `mx-0.5`); `<img>` **`inline-block`**, **`align-[-0.2em]`**, **`object-contain`**, **`ml-1`**, size caps **`max-h-[14px] max-w-[14px] md:max-h-4 md:max-w-4`**, **`h-[1.05em] w-[1.05em]`** — **no `rounded-sm`** on the mark.

- **Gotcha — hero height vs copy**: On **`md+`**, **`min-h-[calc(100dvh-5.25rem)]`** + **`100dvh`** interact with **dynamic** browser chrome when the viewport is wide enough for **`md`**; **very short** viewports or **large** text can still make the hero **taller than one screen** (`min-height` only). On **narrow** viewports, **`min-h-[56dvh]`** sets a **floor**—content can still grow taller; the **launch video** (currently commented) would add height on **all** breakpoints.

- **Gotcha — breakpoint behavior**: Do **not** re-apply **full-viewport** **`min-h-[calc(100dvh-5.25rem)]`** to the **default** (mobile) hero without **`md:`**—that previously caused **excess empty space** above/below centered copy on phones. The **current** pattern is **mobile** **`min-h-[56dvh]`** (short band) vs **`md:`** near-full viewport. **Centering** uses base **`justify-center`** (not mobile **`justify-start`**).
...
- **Purpose**: Presents the same three marketing arcs in **document order** for scanning: **text-side** benchmarking (**Text agents**), **voice-side** benchmarking (**Voice agents**—STT/TTS previews merged in one area), then **simulations**. App routes (**`/stt`**, **`/tts`**, **`/tests`**, **`/simulations`**) are unchanged; only **landing** layout/consolidation differs from older tabbed chrome.
...
- **Data model** (`src/app/page.tsx`): Module **`tabs: LandingTab[]`** — each **`{ id, label, navDescription, sections[] }`** where **`navDescription`** is the **blurb** on the **product-area** **card** buttons only—**desktop** sticky **`nav`**, **mobile** the same **card** rendered **in-flow** **above** each **`landing-{id}`** section (not duplicated inside section bodies). **`sections`** is **`LandingFeatureSection[]`**: **`{ headingBold, headingLight, description, images: string[] }`** plus optional **`quickStart?: { steps; closingHeadline?; comparisonSteps? }`** (**`steps`** / **`comparisonSteps`** → **`LandingQuickStartStep`** **`{ key, headingBold, headingLight, image }`**). **`closingHeadline`** is **`{ headingBold, headingLight, subheading? }`** — optional **`subheading`** renders as **`landingBannerIntroDescriptionClass`** **`p`** under the centered **`h2`** (Text agents today: *Find the best LLM* / *for your agent* plus a one-line comparison blurb before **`comparisonSteps`**). No **`quickStart.heading`** — primary section copy uses **`section` intro fields**. **`Text agents`** intro: *Evaluate the quality* / *of your AI responses*. **`quickStart.comparisonSteps`**: second ladder after optional **`closingHeadline`**; **`QuickStartStepIndicator` `total`** = length of the **array being rendered**. Render order: **`steps`** → **`closingHeadline`** (optional **`h2`** ± **`subheading`**) → **`comparisonSteps`** (optional). **`Text agents`** chains three **`steps`**, closing banner (+ optional subheading), then two **comparison** steps; **`images[]`** mirrors raster order for any code that only reads **`images`**. **`Voice`**: two **`sections`**; **`llm`** / **`simulations`**: one each. Non-**`quickStart`** sections use **`LandingFeatureImageColumn`**.
- **Heading typography** (module constants in **`page.tsx`**): **`landingBannerHeadlineClass`** — large split headings for **quick-start intro** (`h2`), **`closingHeadline`** (`h2`), and **Voice/Simulations** sticky column titles (`h2`): responsive scale through **`xl:text-[3rem]`**, **`w-full md:text-balance`**, inner spans **`font-semibold text-gray-900`** + **`font-light text-gray-500`**. **`md:text-balance`** replaces **unconditional** **`text-balance`** so **narrow** viewports use **normal line wrapping** across the **full content width**; **`text-balance`** alone tended to shorten the **used text box** on phones (especially step **`h3`**s), conflicting with marketing layout. **`landingBannerIntroDescriptionClass`** — the **`<p>`** for **`section.description`** directly under those banner **`h2`**s (Text agents intro, Voice/Sim left column) **and** for **`quickStart.closingHeadline.subheading`** when set (same visual tier as section intro subtitles): **`text-base md:text-lg lg:text-xl`**, **`text-gray-500`**, **`leading-relaxed`**, **`text-pretty`**. When **`closingHeadline.subheading`** exists, the closing **`h2`** gets **`mb-3 md:mb-4`** so the subtitle does not crowd the **`h2`**. Same class for **desktop and mobile** quick-start intros. **`landingStepHeadlineClass`** — one step **smaller** for per-row **`h3`** titles in **`LandingQuickStartDesktopStepStack`** / **`LandingQuickStartMobileStepStack`** (**also** **`w-full md:text-balance`**). Step stacks take prop **`stepHeadlineClass`** (passed **`landingStepHeadlineClass`**). **Obsolete**: **`text-balance`** on these **banner/step** constants **without** **`md:`** — do not revert without checking **mobile** line length. **Obsolete names**: **`featureHeadlineClass`** / **`quickStartClosingHeadlineClass`** — removed; do not reintroduce.
- **Heading hierarchy — `quickStart` (Text agents)**: First **`h2`** (banner class) + **`description`** **`p`** (**`landingBannerIntroDescriptionClass`**), then **`steps`** (**`h3`**, step class), optional **`closingHeadline`** **`h2`** (banner class) with optional **`subheading`** **`p`** on the next line, then optional **`comparisonSteps`** (**`h3`**, step class). The closing **`h2`** remains **larger** than step-row **`h3`** titles; **`subheading`** matches intro subtitle scale, not step titles.
...
- **Gotcha — naming vs Get Started**: The **Evaluate** column still uses **distinct** docs links (**speech-to-text** / **text-to-speech**); not driven by **`tabs`**. **LLM Tests** vs **Text agents** wording mismatch remains where noted for **text-to-text** quickstart.
...
- **Desktop content** (`md+` quick-start): **intro** `h2` → **`steps`** → **`closingHeadline`** **`h2`** (± **`subheading`**) → **`comparisonSteps`** (when set) via **`LandingQuickStartDesktopStepStack`**. Without **`quickStart`**: **`LandingFeatureImageColumn`**; Voice uses **multiple** subsection rows in one **`landing-voice`** section.
...
- **Feature-preview rasters**: **Text agents** — **`quickStart.steps[].image`** + **`quickStart.comparisonSteps[].image`** (mirrored **`images[]` order`): **`llm-input.png`**, **`llm-evaluator.png`**, **`llm-output.png`**, **`llm-multi-input.png`**, **`llm-multi-output.png`**. Unused **`public/`** rasters (**`llm-ui.png`**, etc.) may remain. **Voice / Simulations**: **`LandingFeatureSection.images`** — **`stt-*`**, **`tts-*`**, **`simulation-run.png`**. Each **`/…`** from **`public/`**. Other landing assets often include **`artpark-mark.webp`**, team portraits **`team/*.jpeg`**, **`logo.svg`** / **`logo-dark.svg`**, **`integrations/`** — see **Integrations Section**.
- **Gotcha — image stack order**: Non-**`quickStart`**: **`images[]`** via **`LandingFeatureImageColumn`**—**no carousel**. **Text agents (`quickStart`)**: previews from **`steps[].image`** **and**, when set, **`comparisonSteps[].image`**. On narrow viewports **text stacks above** image. **Vertical** spacing between rows **`gap-14 md:gap-16 lg:gap-20`**. **`alt`**: keyed by **`step.key`**. Keep **`landingBannerHeadlineClass`**, **`landingBannerIntroDescriptionClass`**, and **`landingStepHeadlineClass`** in sync when adjusting marketing type scale. Re-check **`QuickStartStepIndicator`** when **`total`** is **1** or **>4**. Tweak **emerald** tokens with **landing** accents—avoid stray **hex** outside existing patterns.
