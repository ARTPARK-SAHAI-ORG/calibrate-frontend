...
- **Convert to tests** (bulk action on selected traces, `ConvertTracesToTestsDialog` → `POST /traces/convert-to-tests`): the curation payoff. Choose `response` (re-runs the agent and judges the reply; requires ≥1 llm evaluator, pre-seeded with the `default-llm-next-reply` fork) or `tool_call` (asserts the recorded tool calls; enabled only when every selected trace has tool calls, with an "ignore arguments" toggle → `accept_any_arguments`); optionally link the new tests to agents so they're runnable immediately. On success the selection clears and a toast links to `/tests`. Converted tests then plug into the existing run/benchmark flow, whose run results feed the existing `AddRunToLabellingTaskDialog` labelling path — no new eval/labelling code.
...
| Traces          | `GET /traces?limit=&offset=&q=&conversation_id=` (paginated `TraceSummary` envelope, server-side filtering), `GET /traces/{uuid}` (full detail), `POST /traces/bulk-delete` (`{ trace_ids }` or `{ select_all, q?, conversation_id? }`), `POST /traces/convert-to-tests` (`{ trace_ids, type, evaluators?, agent_uuids?, accept_any_arguments? }` → `{ created, test_uuids }`), `GET /org-limits/me/max-traces`. Ingestion (`POST /traces`) is backend/API-key only, not called from this UI                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
