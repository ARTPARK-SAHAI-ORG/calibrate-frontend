...
- **Create test cases** with:
  ...
  - **Evaluators (next-reply tab only)**: Each next-reply test attaches one or more LLM evaluators that score the agent's reply. Evaluators are stored server-side in the `test_evaluators` join table (separate from the test's `config`) and are returned hydrated on `GET /tests` / `GET /tests/{uuid}` as a top-level `evaluators[]` array (joined evaluator + pinned-version row from `get_evaluators_for_test()`).
    ...
    - **Benchmark legacy leaderboard label**: benchmark `leaderboard_summary.pass_rate` is the aggregate score for legacy single-evaluator LLM jobs. Label the table/chart as `<evaluator name> (%)`, resolving the name from `model_results[].test_results[].judge_results[].name` first, then default-evaluator metadata (`default-llm-next-reply` via authenticated or public defaults API). Do not hardcode labels like "Pass rate", "Correctness", or "LLM Judge" in leaderboard UI.
...
- **Per-Simulation Results Table** (shows intermediate results as each simulation completes):
  ...
  - **Desktop** (`hidden md:block`): Table with columns for play button, persona, scenario, and metric columns
    ...
    - Metric columns derived from `runData.metrics` keys, or from `simulation_results[].evaluation_results` when metrics is null. Column headers show the snapshotted evaluator description when available, using the same `evaluatorDescriptionByName` map as the overview cards.
...
- `BenchmarkResultsDialog` also uses the job payload's first `judge_results[].name`, falling back to this helper, to label legacy benchmark leaderboard `pass_rate` as `<evaluator name> (%)`. Do not hardcode benchmark score labels.
...
- `STTEvaluationLeaderboard` / `TTSEvaluationLeaderboard` render the comparative leaderboard table and charts.
...
- Public STT/TTS pages use this metadata when the result payload lacks enough evaluator metadata and must synthesize the legacy `llm_judge_score` column. Public benchmark pages use `type="llm"` as the fallback label source when `model_results[].test_results[].judge_results[].name` is absent. Do not hardcode default evaluator names or descriptions in public pages; use job snapshot fields first, then this helper, then render empty/generic UI only if both are unavailable.
