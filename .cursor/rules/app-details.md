...
- **Empty state** (`TracesEmptyState`): a three-step setup guide in the Sentry mould, shown until this agent's first trace arrives.
  1. **Create an API key** — reuses the workspace-settings pieces unchanged: `useWorkspaceApiKeys(accessToken, orgUuid).createApiKey` behind the shared `CreateApiKeyDialog`. The parent captures the plaintext `key` off the create promise (the backend returns it only once) and holds it in component state — never stored, gone when the screen goes. A link to `/workspace-settings` covers the "already have one" case.
  2. **Send one request per turn** — a two-column split (same `md:grid md:grid-cols-2` as `AgentConnectionTabContent`): left is a language picker (cURL / Python / JavaScript / Go, `src/components/traces/ingestSnippets.ts`) over a `CodeBlock` with the copy button; right is the `SNIPPET_FIELDS` table explaining the address, `X-API-Key`, `agent_id`, `message_id`, `conversation_id`, `input`, `output`. Every snippet is pre-filled with the resolved backend URL, this agent's uuid, and the key from step 1 (`sk_...` until one is made).
  3. **Wait for the first trace** — a spinner. `TracesTabContent` polls `refetch` every `POLLING_INTERVAL_MS` **only while the empty state is showing**, so the list replaces the guide on its own and nothing polls once traces exist.
