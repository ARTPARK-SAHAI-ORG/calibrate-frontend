...
Production agent turns ingested from the customer's backend via `POST /traces` (API-key authed — the backend endpoint, not this UI; the ingest body carries the agent's uuid as a top-level `agent_id`). **Traces belong to one agent**: they are a tab on the agent detail page, placed after **Tests**, and there is no `/traces` route and no sidebar entry for them. The tab is a **read + curate** surface: there is no ingest form. Files: `src/components/agent-tabs/TracesTabContent.tsx`, `src/components/traces/{TracesTable,TraceDetailDialog,TracesEmptyState,ConvertTracesToTestsDialog}.tsx`, `src/lib/tracesApi.ts`, and the `useTraces`/`useTraceCount`/`useMaxTraces`/`useTraceDeletion` hooks.

- **Agent-scoped reads.** Every call carries the agent: `GET /traces?agent_id=…` for the list, the same `agent_id` on the `limit=1` count probe, and `agent_id` in the `select_all` body of `POST /traces/bulk-delete`. `useTraces` and `useTraceCount` both take `agentId` and refetch when it changes, so switching agents never shows another agent's traces.
- **Server-side pagination (the deliberate exception).** Every other list fetches everything and filters client-side; traces are machine-written and can far outgrow that, so `useTraces` drives `limit`/`offset`/`q`/`conversation_id` (plus `agent_id`) on `GET /traces` and holds only one page. `q` (debounced 300ms) and the conversation filter round-trip to the backend. Do not convert this to the fetch-everything pattern. `GET /traces` returns the standard `Paginated<T>` envelope of slim `TraceSummary` rows (previews + `turn_count`/`tool_call_count`/`metadata_count`, no bodies).
...
- **No usage indicator.** There is deliberately no "N / max traces stored" line: an agent-scoped count next to a workspace-wide cap reads as plenty of room left when the workspace is full, and a workspace count on one agent's tab is noise. `useTraceCount`, `useMaxTraces`, `GET /org-limits/me/max-traces` and `LIMITS.DEFAULT_MAX_TRACES` were all removed with it — don't reintroduce one without solving that mismatch.
...
- **Deep-links**: alongside the tab's own `?tab=traces`, `?conversation_id=` (filter, with a clearable chip) and `?traceId=` (open detail, via `useDialogUrlParam`) so a filtered view or an open trace survives reload and is shareable.
...
- **Convert to tests** (bulk action on selected traces, `ConvertTracesToTestsDialog` → `POST /traces/convert-to-tests`): the curation payoff. Choose `response` (re-runs the agent and judges the reply; requires ≥1 llm evaluator, pre-seeded with the `default-llm-next-reply` fork) or `tool_call` (asserts the recorded tool calls; enabled only when every selected trace has tool calls, with an "ignore arguments" toggle → `accept_any_arguments`). The dialog does **not** ask which agents to link: it takes `agentUuid` and always sends `agent_uuids: [agentUuid]`, so the new tests belong to the agent whose tab this is and are runnable immediately. On success the selection clears and a toast counts `test_uuids` ("Created N tests") and links to them. Converted tests then plug into the existing run/benchmark flow, whose run results feed the existing `AddRunToLabellingTaskDialog` labelling path — no new eval/labelling code.
...
| Traces          | `GET /traces?agent_id=&limit=&offset=&q=&conversation_id=` (paginated `TraceSummary` envelope, server-side filtering; `agent_id` is required — every read is scoped to one agent), `GET /traces/{uuid}` (full detail, includes `agent_id`), `POST /traces/bulk-delete` (`{ trace_ids }` or `{ select_all, agent_id, q?, conversation_id? }`), `POST /traces/convert-to-tests` (`{ trace_ids, type, evaluators?, agent_uuids?, accept_any_arguments? }` → `{ test_uuids }`, no count: the "created N tests" message counts `test_uuids`; `agent_uuids` is always the one agent whose tab the user is on), `GET /org-limits/me/max-traces`. Ingestion (`POST /traces`, body carries top-level `agent_id`) is backend/API-key only, not called from this UI                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
