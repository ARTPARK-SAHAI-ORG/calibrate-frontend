...
- **Actions**: Add test (button in tests table header), Run all tests (header action button, limit fetched from backend per user — sends no test ids so the backend runs every linked test; with rows ticked it reads "Run N tests" and sends the ticked ids, asking "Start the run" first either way), Run single test (row button — sends `test_uuids: [uuid]`), Compare models (benchmark — sends only `models` and no `test_uuids` with nothing ticked, the ticked ids otherwise), Delete selected (header button shown while rows are ticked, calls `POST /agent-tests/bulk-delete-tests` once with every selected uuid). While rows are ticked the header Run button reads "Run N tests" and, like Compare models, acts on the ticked tests only; the selection strip above the table holds just the count, "Select all N tests" and Clear. Deleting a test from the agent page deletes it from the workspace: there is no detach-only action here, so the test also goes from every other agent that uses it
...
- **API**: Fetches runs from `GET /agent-tests/agent/{uuid}/runs`; deletes one with `DELETE /agent-tests/job/{task_id}` (the same id the run is polled with, and the one route for a plain run and a model comparison alike). There is no bulk delete for runs, so the Evaluations tab deletes one row at a time, and only once the run has finished: the delete icon is greyed out while a run is queued or in progress, because deleting it would remove the record while the run itself kept going
...
- **Bulk upload tests** via CSV:
  ...
  - **Step 1**: "Select what you want to test about the agent" — two option cards named after the question each answers, matching the create test flow: "Does the agent give the right reply?" (a `general` agent gets "Does the agent give the right answer?") and "Does the agent use the right tool?". The selected card is filled (`bg-foreground text-background border-foreground`).
...
### 11. Traces (agent detail tab, `/agents/[uuid]?tab=traces`)

Production agent turns ingested from the customer's backend via `POST /traces` (API-key authed — the backend endpoint, not this UI; the ingest body carries the agent's uuid as a top-level `agent_id`). `message_id` and `conversation_id` are optional and may be omitted or sent as `null`. `message_id` is still expected and recommended as the unique identifier for the customer's message, but duplicate values are accepted and create separate traces. The response has no `created` field. **Traces belong to one agent**: they are a tab on the agent detail page, placed after **Tests**, and there is no `/traces` route and no sidebar entry for them. The tab is a **read + curate** surface: there is no ingest form. Files: `src/components/agent-tabs/TracesTabContent.tsx`, `src/components/traces/{TracesTable,TraceDetailDialog,TracesEmptyState,ConvertTracesToTestsDialog,TraceScoringSummary,TraceScorePanel}.tsx`, `src/lib/tracesApi.ts`, `src/lib/traceScoring.ts`, and the `useTraces`/`useTraceDeletion`/`useAgentTraceScoring` hooks.

- **Trace scoring.** The **Score new traces automatically** switch lives in a Traces section on the agent **Settings** tab, The switch sits under a **Traces** heading on the Settings tab, and when no evaluator can score it is disabled with the reason on hover rather than an extra line. The setting is read out of the agent's own config by `traceScoringEnabled`, on unless it was turned off, the same rule the backend uses, rather than from a field beside the config that not every answer carries. It is changed by saving the agent's stored config back with `traces.scoring.enabled` set. A save replaces the whole config rather than merging it, so `configWithTraceScoring` in `src/lib/tracesApi.ts` is the one place that knows the path and carries everything else over. It is on by default for every agent. Which linked evaluators can score comes from JWT-only `GET /agents/{uuid}/trace-scoring-eligibility`; turning the switch on is refused with a 422 when none can. The ones that cannot are named with their reason (`wrong_type_for_agent`, `no_live_version`, `declares_variables`, in ordinary words). `useAgentTraceScoring` is called once in `AgentDetail` and handed to both tabs as `TraceScoringControls`. Once traces exist, the Traces tab shows one line saying whether scoring is on, with a button that opens the Settings tab where the switch lives and a second line naming the evaluators that cannot score traces and why. When any trace on the page carries `latest_run_error: "over_limit"`, a third line says the workspace has scored as many traces as its limit allows, with the same contact link the limit toast uses (`CONTACT_LINK`). How the scoring of a trace went is a mark beside its input, the way a run's mark sits beside its name (`TraceScoreMark`, reusing `RunStateMark` with its own words): a tick when scored, a spinner while it waits or runs, and an amber or red mark naming the reason on hover through `scoringRunErrorCopy`. The evaluator columns hold results only, and are blank when a run produced none. A trace nothing has tried to score gets no scores column in its window at all. A column is drawn for every evaluator that can score now and for any that scored a trace on the page, so taking an evaluator off the agent does not hide the scores it already gave. When no linked evaluator can score, the line says so instead, and a Choose the evaluators button opens the agent's Evaluators tab. The number in that line comes from `GET /traces/usage`, asked for only once a trace on the page says `over_limit`, so the page makes no request until the cap has actually bitten.
- **Agent-scoped reads.** Every list call carries `agent_id` on `GET /traces`. `useTraces` takes `agentId` and refetches when it changes, so switching agents never shows another agent's traces. **Search is server-side**: the tab's search box feeds `q` on `GET /traces`, a plain case-insensitive "contains this text" match over the message id, conversation id, conversation history, reply, and metadata. A page is capped at `MAX_TRACES_PAGE_SIZE` (200), which `fetchTraces` clamps to.
- **Server-side pagination (the deliberate exception).** Every other list fetches everything and filters client-side; traces are machine-written and can far outgrow that, so `useTraces` drives the complete supported query contract, `agent_id` + `q` + `limit`/`offset`, on `GET /traces` and holds only one page. `q` is debounced by 300ms and round-trips to the backend. Do not convert this to the fetch-everything pattern. `GET /traces` returns the standard `Paginated<T>` envelope of slim `TraceSummary` rows (previews + `tool_names` / `tool_calls` + `turn_count`/`tool_call_count`/`metadata_count`, plus `latest_run_status` and `results`, one entry per evaluator from the latest completed scoring run in the same shape `GET /traces/{uuid}/scores` uses (the table reads `evaluator_uuid`, `output_type`, `value` and `passed`); no bodies).
- **List** (`TracesTable`): desktop table / mobile cards. The table columns are **Input** (`input_preview` as the primary text, with `message_id` underneath only when present), **Output** (`response_preview`, or the tool name plus a compact arguments line from `tool_calls` when there is no text reply, falling back to joined `tool_names` — never a "Tool calls only" placeholder), one column per evaluator that can score (from the eligible list, reading each row's `results`; a spinner spans those columns while the trace is waiting or being scored, and a failed or skipped run is a pill), and **Created**, plus selection and delete controls. There are no conversation, turn-count, or tool-count columns. Pagination uses the shared **server-paginated list bar** (see `CLAUDE.md` → _Server-paginated list bar_): count on the left, **Per page** + icon prev/next + **Page X of Y** on the right when `total > 10`, in one row **above** the table. Rows open the detail dialog. While any row on the page is still being scored, `useTraces` asks for the page again every 3 seconds (`POLLING_INTERVAL_MS`), one list request, not one per row.
- **Detail** (`TraceDetailDialog`): fetches its own full trace (`fetchTrace`) so the list stays slim, and `GET /traces/{uuid}/scores` for the scoring runs, of which only the latest is shown. Heading is the last user turn (or "Trace" if none). The body reuses `TestDetailView` from test-run results: chat history, then the agent's final output emphasised without pass/fail chrome (`showVerdict={false}`). The window is three columns: metadata, the input and output, then one verdict card per evaluator from the latest run (`TraceScorePanel`, given that one run, drawing `EvaluatorVerdictCard`, with the stored numeric `value` split back into the card's match/score by `output_type`). The metadata column (stacked on mobile) holds: `message_id` labelled **Name** and `conversation_id` labelled **Conversation** when present (omitted entirely when absent — no "no id" copy), always **Created**, then ingest metadata entries.
- **No usage indicator.** There is deliberately no "N / max traces stored" line: an agent-scoped count next to a workspace-wide cap reads as plenty of room left when the workspace is full, and a workspace count on one agent's tab is noise. Do not reintroduce a usage meter without solving that mismatch.
- **Delete**: single (row trash icon) and bulk (select checkboxes → "Delete selected (N)") both go through `POST /traces/bulk-delete` with `trace_ids` (via `useTraceDeletion` over the shared `useBulkDeletion`); there is no per-trace DELETE. The only accepted body is `{ trace_ids }` — no `select_all`, no filters, and unknown fields are rejected — so there is no "delete everything matching" path. Deletes are soft and free workspace capacity.
- **Deep-links**: alongside the tab's own `?tab=traces`, `?traceId=` opens a trace detail via `useDialogUrlParam`, so an open trace survives reload and is shareable.
- **Empty state** (`TracesEmptyState`): a three-step setup guide in the Sentry mould, shown until this agent's first trace arrives. **One step at a time**: a later step is not rendered at all until the one before it is finished, and a finished step collapses to its heading (click to reopen; opening one closes the others). `reached` tracks the furthest step, `openStep` what is expanded. Each card copies the Connection check card (`border border-border rounded-xl p-3 md:p-4 space-y-3`).
  1. **Create an API key** — reuses the workspace-settings pieces unchanged: `useWorkspaceApiKeys(accessToken, orgUuid).createApiKey` behind the shared `CreateApiKeyDialog`. The parent captures the plaintext `key` off the create promise (the backend returns it only once) and holds it in component state — never stored, gone when the screen goes. Closing the dialog's reveal advances to step 2. **I have a key already** is the escape hatch: paste the key and check it with `validateApiKeyForAgent` (`GET /agents/{uuid}` with `X-API-Key` only — not `apiGet`, which would attach the session JWT and sign the user out on 401). A 2xx fills the same in-memory key and advances to step 2, identical to creating one. 401/403/404 stay on step 1 with "This key did not work. Check it is for this workspace." Reopening step 1 shows the key and **Create a new API key**.
  2. **Send your first trace** — a two-column split, `lg:grid-cols-[3fr_2fr]` so a key and a URL fit on one code line. Left is one object: a header bar with the language tabs (cURL / Python / JavaScript, from `SNIPPET_FIELDS`' sibling `SNIPPET_LANGUAGES` in `src/components/traces/ingestSnippets.ts`) on the left and the copy button on the right, over the `<pre>` — the industry shape, per Sentry/Resend/Clerk. Right is the `SNIPPET_FIELDS` list: `agent_id`, `input`, `output` first, then `message_id`, `conversation_id`, `metadata` under an **Optional** heading. Both ids may be omitted or `null`; when supplied, `message_id` should uniquely identify the customer's message even though the backend accepts duplicates as separate traces. Snippets carry no per-field commentary (just one `# Optional` marker) and are pre-filled with the resolved backend URL, this agent's uuid, and the key from step 1 (`sk_...` until one is made). `output` shows BOTH `response` and `tool_calls` so the tool-call shape is visible; the field note says to send at least one. All JSON is wrapped so nothing scrolls sideways. "I have added this" advances to step 3.
  3. **Check that it arrived** — a pulsing amber dot with "No traces for this agent yet" and a primary **Check for traces** button that calls the tab's `refetch` (passed in as `onCheckForTraces`). **Nothing polls**: the reader says when to look. The tab swaps this whole screen for the list once traces come back, so the only outcome handled here is "still nothing".
- **Code highlighting**: a ~15-line regex inside `TracesEmptyState.tsx` that colours strings and comments only. Deliberately not a highlighter dependency, and deliberately not its own file — three fixed snippets in one screen justify neither.
- **Add to tests** (the user-facing bulk action on selected traces; internally `ConvertTracesToTestsDialog` + `convertTracesToTests` → `POST /traces/convert-to-tests`): there is no type picker. `TracesTabContent` derives `tool_call` only when every selected trace has no `response_preview` and has `tool_call_count > 0`; otherwise it derives `response`, so any selected trace with a response forces response mode. Response mode shows the LLM evaluator selection and preselects the `default-llm-next-reply` fork; at least one evaluator is required. Tool-call mode shows the **Match tool name only (ignore arguments)** option, which maps to `accept_any_arguments`. The dialog does **not** ask which agents to link: the backend links each created test to the agent that produced its trace, and `agentUuid` only decides which evaluators are offered. The backend owns generated test names and the naming fallback; the frontend sends no name. On success the selection clears and a toast shows the returned `created` count ("Created N tests") and links to them. On failure `convertTracesErrorMessage` pulls the backend's own message (which evaluators cannot be used, or which traces have no tool calls or no longer exist) out of the thrown error and the dialog shows it; nothing is created, so the reader can fix it and resubmit. Added tests then plug into the existing run/benchmark flow, whose run results feed the existing `AddRunToLabellingTaskDialog` labelling path — no new eval/labelling code.
...
15. **View Mode Dialogs**: `TestRunnerDialog` and `BenchmarkResultsDialog` support dual modes:
    ...
    - **BenchmarkResultsDialog intermediate results**:
      ...
      - **When done**: Shows both the Results and Tests tabs. A comparison opened from the runs list (a `taskId` was passed and it had already finished) lands on Results; one started and watched in this window stays on Tests. `TestRunnerDialog` follows the same rule.
...
| Agent Tests     | `GET /agent-tests/agent/{uuid}/tests`, `GET /agent-tests/agent/{uuid}/runs`, `DELETE /agent-tests/job/{taskId}`, `POST/DELETE /agent-tests`, `POST /agent-tests/bulk-unlink`, `POST /agent-tests/bulk-delete-tests`, `POST /agent-tests/agent/{uuid}/run`, `GET /agent-tests/run/{taskId}`, `POST /agent-tests/agent/{uuid}/benchmark`, `GET /agent-tests/benchmark/{taskId}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Traces          | `GET /traces?agent_id=&limit=&offset=` (paginated `TraceSummary` envelope; `agent_id` is required, `q`, `output_type` and `labels` narrow it, and `limit` above 200 is refused), `GET /traces/{uuid}` (full detail, includes `agent_id`), `GET /traces/{uuid}/scores` (the scoring runs newest first, with per-evaluator `value`/`output_type`/`passed`; the UI reads the latest only), JWT-only `GET /agents/{uuid}/trace-scoring-eligibility`, `PUT /agents/{uuid}` sending the stored config back with `traces.scoring.enabled` set (422 when no linked evaluator can score), `GET /traces/usage` (how much of the stored and scored limits is used), `POST /traces/bulk-delete` (`{ trace_ids }` only; unknown fields are rejected), `POST /traces/convert-to-tests` (`{ trace_ids, type, evaluators?, accept_any_arguments? }` → `{ created, test_uuids }`, ids in the order the traces were sent; `evaluators` is a plain list of evaluator uuids, allowed only for evaluators with no variables and `evaluator_type` `llm`; up to 500 traces a request; each created test is linked by the backend to the agent that produced its trace, so no agent is sent; generated names and their fallback are backend-owned; a failure creates nothing and its `detail` names the evaluators or `trace_ids` at fault). Ingestion (`POST /traces`, body carries top-level `agent_id`) is backend/API-key only, not called from this UI; `message_id` / `conversation_id` are optional and nullable, duplicate `message_id` values create separate traces even though the field is recommended as the customer's unique message identifier, and the response has no `created` field |
