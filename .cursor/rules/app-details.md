...
- **Usage indicator**: `count / max_traces` at the top of the tab — `useTraceCount` (a filter-independent `limit=1` probe for this agent, reading the envelope `total`) over `useMaxTraces` (`GET /org-limits/me/max-traces`, `DEFAULT_MAX_TRACES` fallback). Makes the backend's 429 storage cap visible before it's hit.
...
- **Empty state** (`TracesEmptyState`): feature blurb + copy-paste `curl` ingest snippet against the resolved backend URL, with this agent's uuid already filled in as `agent_id`, linking to `/workspace-settings` for the API key.
