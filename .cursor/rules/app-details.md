...
### 11. Traces (agent detail tab, `/agents/[uuid]?tab=monitoring`)
...
- **Continuous monitoring.** The switch **Enable continuous monitoring** sits under a **Monitoring** heading on the agent Settings tab, and asks for confirmation before it changes anything. It is disabled, with the reason on hover, when no linked evaluator can score. The setting is read out of the agent's own config by `traceScoringEnabled`, on unless it was turned off, the same rule the backend uses, rather than from a field beside the config that not every answer carries. It is changed by saving the agent's stored config back with `traces.scoring.enabled` set; a save replaces the whole config rather than merging it, so `configWithTraceScoring` in `src/lib/tracesApi.ts` is the one place that knows the path. Which linked evaluators can score comes from `GET /agents/{uuid}/trace-scoring-eligibility`. `useAgentTraceScoring` is called once in `AgentDetail` and handed to both tabs as `TraceScoringControls`. The Monitoring tab stays quiet while scoring works: the state is a pill on the count row above the table (`TraceScoringChip`), green when on and amber when off, which toggles with a confirmation and shows on hover which evaluators score and which cannot. Only a real problem takes a row of its own: no evaluator can score, or a trace carries `latest_run_error: "over_limit"`, whose line names the cap from `GET /traces/usage` and carries the contact link the limit toast uses (`CONTACT_LINK`). Above everything, `EvaluatorScoreCards` shows one live card per evaluator, added up in the browser from the traces on the page until the backend reports them. How the scoring of one trace went is a mark beside its input (`TraceScoreMark`, reusing `RunStateMark` with its own words), with the reason on hover through `scoringRunErrorCopy`. The evaluator columns hold results only: a verdict or a score, an information icon carrying the judge's reasoning, "In progress" while the run is going, and "Not scored yet" otherwise. A column is drawn for every evaluator that can score now and for any that scored a trace on the page, so taking an evaluator off the agent does not hide the scores it already gave. - **Server-side pagination (the deliberate exception).** Every other list fetches everything and filters client-side; traces are machine-written and can far outgrow that, so `useTraces` drives the complete supported query contract, `agent_id` + `q` + `limit`/`offset`, on `GET /traces` and holds only one page. `q` is debounced by 300ms and round-trips to the backend. Do not convert this to the fetch-everything pattern. `GET /traces` returns the standard `Paginated<T>` envelope of slim `TraceSummary` rows (previews + `tool_names` / `tool_calls` + `turn_count`/`tool_call_count`/`metadata_count`, plus `latest_run_status` and `results`, one entry per evaluator from the latest completed scoring run in the same shape `GET /traces/{uuid}/scores` uses (the table reads `evaluator_uuid`, `output_type`, `value` and `passed`); no bodies).
...
- **Deep-links**: alongside the tab's own `?tab=monitoring`, `?traceId=` opens a trace detail via `useDialogUrlParam`, so an open trace survives reload and is shareable.
