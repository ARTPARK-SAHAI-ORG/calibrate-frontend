...
- **Spreadsheet export (XLSX)**: `xlsx` (SheetJS ~0.18.x) — human labelling **evaluation run detail** only; lazy `import("xlsx")`. After import use **`xlsxMod.default ?? xlsxMod`** — the ESM build has no **`default`**, so **`.default` alone** breaks at runtime (e.g. Turbopack). In-app entry point: primary **Export results** control (tooltip **Download spreadsheet (XLSX)**); per-evaluator sheets include **Human agreement** / **Evaluator agreement** columns (API or computed — see **Human Labelling → Evaluation run detail**).
- **TypeScript**: 5.x
...
│   │   ├── human-labelling/   # Labelling dialogs, AnnotationJobView, bulk-upload shell + shared types (`bulk-upload-shared.tsx`, e.g. `AnnotatedCheckResult`)
...
7. **CSV export and share header actions**:
   ...
   - **Excel (`.xlsx`) — human labelling evaluation run detail only:** Multi-sheet exports use **`xlsx`** (SheetJS) with a **dynamic `import("xlsx")`** in `src/app/human-labelling/tasks/[uuid]/evaluator-runs/[runUuid]/page.tsx` — **not** `ExportResultsButton` / `DownloadableTable` (those are CSV). Row schema includes **Human agreement** and **Evaluator agreement** before machine/annotator value columns (**`agreementExportCell`**, optional API fields on **`human_agreement.items[].evaluators[]`** — full column order and fallbacks under **Human Labelling → Evaluation run detail**). The page exposes a **primary-styled** **Export results** button next to the run status when the job is **completed** and items exist; **`title`** carries the XLSX hint. **Interop gotcha** and the required **`default ?? namespace`** pattern, plus full **button** rules, are in the same Evaluation run detail subsection (this file).
