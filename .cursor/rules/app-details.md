...
- **Agent-scoped reads.** Every list call carries `agent_id` on `GET /traces`. `useTraces` takes `agentId` and refetches when it changes, so switching agents never shows another agent's traces. There is **no search**: the endpoint takes no query term. A page is capped at `MAX_TRACES_PAGE_SIZE` (200), which `fetchTraces` clamps to.
