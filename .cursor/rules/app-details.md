...
- **Server-side pagination (the deliberate exception).** Every other list fetches everything and filters client-side; traces are machine-written and can far outgrow that, so `useTraces` drives the complete supported query contract, `agent_id` + `q` + `limit`/`offset`, on `GET /traces` and holds only one page. `q` is debounced by 300ms and round-trips to the backend. Do not convert this to the fetch-everything pattern. `GET /traces` returns the standard `Paginated<T>` envelope of slim `TraceSummary` rows (previews + `tool_names` / `tool_calls` + `turn_count`/`tool_call_count`/`metadata_count`, plus the latest scoring run's `latest_run_status` / `passed` / `n_passed` / `n_total`, no bodies).
- **List** (`TracesTable`): desktop table / mobile cards. The table columns are **Input** (`input_preview` as the primary text, with `message_id` underneath only when present), **Output** (`response_preview`, or the tool name plus a compact arguments line from `tool_calls` when there is no text reply, falling back to joined `tool_names` — never a "Tool calls only" placeholder), **Scores** (latest-run status, or for a completed run the same `N Success` / `M Fail` count pills as the Evaluations Result cell — never an average of mixed evaluator types), and **Created**, plus selection and delete controls. There are no conversation, turn-count, or tool-count columns. Pagination uses the shared **server-paginated list bar** (see `CLAUDE.md` → _Server-paginated list bar_): count on the left, **Per page** + icon prev/next + **Page X of Y** on the right when `total > 10`, in one row **above** the table. Rows open the detail dialog. While the tab is on screen and any visible row is `pending` or `processing`, `useTraces` refetches that page every `POLLING_INTERVAL_MS` (3s) — one list request, not one per row.
- **Detail** (`TraceDetailDialog`): fetches its own full trace (`fetchTrace`) so the list stays slim, and `GET /traces/{uuid}/scores` for every scoring run newest-first. Heading is the last user turn (or "Trace" if none). The body reuses `TestDetailView` from test-run results: chat history, then the agent's final output emphasised without pass/fail chrome (`showVerdict={false}`). Below that, `TraceScoreHistory` shows pending/processing/completed/failed/skipped runs and reuses `EvaluatorVerdictCard` for each evaluator's binary or rating result (the stored numeric `value` is split back into the card's match/score by `output_type`), reasoning, scale, and version. The right-hand column (stacked below on mobile) holds metadata only: `message_id` labelled **Name** and `conversation_id` labelled **Conversation** when present (omitted entirely when absent — no "no id" copy), always **Created**, then ingest metadata entries.
