...
- **List** (`TracesTable`): desktop table / mobile cards. The table columns are **Input** (`input_preview` as the primary text, with `message_id` underneath only when present), **Output** (`response_preview`, or the tool name plus a compact arguments line from `tool_calls` when there is no text reply, falling back to joined `tool_names` — never a "Tool calls only" placeholder), and **Created**, plus selection and delete controls. There are no conversation, turn-count, or tool-count columns. Pagination uses the shared **server-paginated list bar** (see `CLAUDE.md` → *Server-paginated list bar*): count on the left, **Per page** + icon prev/next + **Page X of Y** on the right when `total > 10`, in one row **above** the table. Rows open the detail dialog.
