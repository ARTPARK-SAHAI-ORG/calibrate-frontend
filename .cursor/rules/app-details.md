...
- **List** (`TracesTable`): desktop table / mobile cards. The table columns are **Input** (`input_preview` as the primary text, with `message_id` underneath only when present), **Output** (`response_preview`, or the tool name plus a compact arguments line from `tool_calls` when there is no text reply, falling back to joined `tool_names` — never a "Tool calls only" placeholder), and **Created**, plus selection and delete controls. There are no conversation, turn-count, or tool-count columns. The total trace count appears above the table; **Previous** / **Next** pagination controls appear below it. Rows open the detail dialog.
