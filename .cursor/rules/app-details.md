...
- **Agent-scoped reads.** Every list call carries `agent_id` on `GET /traces`, and the `select_all` body of `POST /traces/bulk-delete` carries the same agent id. `useTraces` takes `agentId` and refetches when it changes, so switching agents never shows another agent's traces.
...
- **Delete**: single (row trash icon) and bulk (select checkboxes → "Delete selected (N)") both go through `POST /traces/bulk-delete` with `trace_ids` (via `useTraceDeletion` over the shared `useBulkDeletion`); there is no per-trace DELETE. When a search is active, a **"Delete all N matching"** button uses the backend `select_all` contract to remove every matching search result across pages, not just the loaded ones (`bulkDeleteMatchingTraces`). Accepted bodies are `{ trace_ids }` or `{ select_all: true, agent_id, q? }`; unknown fields are rejected. Deletes are soft and free workspace capacity.
...
- **Add to tests** (the user-facing bulk action on selected traces; internally `ConvertTracesToTestsDialog` + `convertTracesToTests` → `POST /traces/convert-to-tests`): there is no type picker. `TracesTabContent` derives `tool_call` only when every selected trace has no `response_preview` and has `tool_call_count > 0`; otherwise it derives `response`, so any selected trace with a response forces response mode. Response mode shows the LLM evaluator selection and preselects the `default-llm-next-reply` fork; at least one evaluator is required. Tool-call mode shows the **Match tool name only (ignore arguments)** option, which maps to `accept_any_arguments`. The dialog does **not** ask which agents to link: it takes `agentUuid` and always sends `agent_uuids: [agentUuid]`, so the new tests belong to the agent whose tab this is and are runnable immediately. The backend owns generated test names and the naming fallback; the frontend sends no name. On success the selection clears and a toast counts `test_uuids` ("Created N tests") and links to them. Added tests then plug into the existing run/benchmark flow, whose run results feed the existing `AddRunToLabellingTaskDialog` labelling path — no new eval/labelling code.
...
| Traces          | `GET /traces?agent_id=&q=&limit=&offset=` (paginated `TraceSummary` envelope; these are the only supported query fields and `agent_id` is required), `GET /traces/{uuid}` (full detail, includes `agent_id`), `POST /traces/bulk-delete` (`{ trace_ids }` or `{ select_all: true, agent_id, q? }`; unknown fields are rejected), `POST /traces/convert-to-tests` (`{ trace_ids, type, evaluators?, agent_uuids?, accept_any_arguments? }` → `{ test_uuids }`, no count: the "created N tests" message counts `test_uuids`; `agent_uuids` is always the one agent whose tab the user is on; generated names and their fallback are backend-owned). Ingestion (`POST /traces`, body carries top-level `agent_id`) is backend/API-key only, not called from this UI; `message_id` / `conversation_id` are optional and nullable, duplicate `message_id` values create separate traces even though the field is recommended as the customer's unique message identifier, and the response has no `created` field                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
