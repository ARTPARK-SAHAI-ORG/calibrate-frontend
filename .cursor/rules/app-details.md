...
│   │   ├── page.tsx           # Landing page (marketing: responsive hero — shorter min-height band on mobile; `md+` near-full-viewport hero; open-source pill; institutional eyebrow; desktop sticky product-area nav + scroll spy; below `md` each product tab renders as in-flow nav button then section; **`tabs`** drives Text agents / Voice agents / Simulations — all three use **`quickStart`** step ladders via **`LandingQuickStartDesktopStepStack`** / **`LandingQuickStartMobileStepStack`**; integrations, community, etc.)
