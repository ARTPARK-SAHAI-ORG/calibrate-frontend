...
type VersionCardProps = {
  ...
  /** Rendered as the details for a version the reader picked from a list:
   *  the prompt shows straight away, there is no show/hide button, and the
   *  version number and Current pill are left to the list. */
...
export function VersionCard({
  ...
  const [promptVisible, setPromptVisible] = useState(
    isLive || isDefault || isSelectedDetail,
  );
  ...
  // Where the card is the details pane, that pane scrolls, so the prompt is
  // shown whole rather than cut off behind a "View more" button.
  const clampPrompt = isPromptOverflowing && !isSelectedDetail;
  const hasJudgeModel = !!version.judge_model?.trim();
  const hasPrompt = !!version.system_prompt?.trim();
  const showsVersionChrome = !isDefault && !isSelectedDetail;
  const showHeaderRow = hasJudgeModel || showsVersionChrome;
  ...
  useEffect(() => {
    setPromptVisible(isLive || isDefault || isSelectedDetail);
  }, [isLive, isDefault, isSelectedDetail]);
  ...
  return (
    ...
    >
      {/* The whole header row goes when it would be empty: the version pill
          and the buttons only appear on the evaluator's own page, and an
          evaluator no AI judge runs has no model to name. Left in, it was an
          empty row with the card's gap above and below it. */}
      {showHeaderRow && (
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex flex-col gap-1 min-w-0">
            ...
            {/* An evaluator that no AI judge runs, such as Tool call
              correctness, has no model and no prompt. Show nothing rather
            ...
            {hasJudgeModel && (
              <>
                ...
                <code className="text-sm md:text-base font-mono font-medium text-foreground truncate">
                  {version.judge_model}
                </code>
              </>
            )}
      ...
      {promptVisible &&
      outputType === "rating" &&
      version.output_config?.scale?.length ? (
        <div className="space-y-2">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
            Output
          </div>
