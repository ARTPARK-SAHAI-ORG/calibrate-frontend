  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      >
        ...
        {/* Talk to Us - pinned to the bottom of the sidebar */}
        {floatingButtonHideCount === 0 && (
          ...
          >
            ...
            {talkToUsOpen && (
              <div
                className={`absolute bottom-full mb-2 w-56 bg-background border border-border rounded-xl shadow-lg overflow-hidden z-[9999] ${
                  sidebarOpen ? "left-3" : "left-2"
                }`}
              >
                <div className="p-2">
                  <a
            ...
            {/* Button - full when expanded, icon-only when collapsed */}
            {sidebarOpen ? (
              <button
                onClick={() => setTalkToUsOpen(!talkToUsOpen)}
                className={`w-full h-10 rounded-md flex items-center gap-2.5 px-2 text-sm font-medium transition-colors cursor-pointer ${
                  talkToUsOpen
                    ? "bg-accent text-accent-foreground"
                    : "text-muted-foreground hover:bg-accent/50 hover:text-foreground"
                }`}
