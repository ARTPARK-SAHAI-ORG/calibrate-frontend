...
export function TextToSpeechEvaluation({
  ...
  const [availableEvaluators, setAvailableEvaluators] = useState<
    EvaluatorData[]
  >([]);
  const [selectedEvaluatorUuids, setSelectedEvaluatorUuids] = useState<
    string[]
  >([]);
  ...
  const handleEvaluatorsChange = (uuids: string[]) => {
    setSelectedEvaluatorUuids(uuids);
    if (uuids.length > 0) setEvaluatorsInvalid(false);
  };
  ...
  useEffect(() => {
    const fetchEvaluators = async () => {
      ...
      try {
        ...
        const ttsEvaluators = unwrapList<EvaluatorData>(data).filter(
          (m) => m.evaluator_type === "tts",
        );
    ...
    fetchEvaluators();
  }, [backendAccessToken, evaluatorsReloadKey]);
  ...
  const handleLanguageChange = (newLanguage: LanguageOption) => {
    ...
    const newFilteredProviders = getFilteredProviders(
      newLanguage,
      enabledProviders,
    );
  ...
  const handleEvaluate = async () => {
    ...
    // Validate evaluators
    if (selectedEvaluatorUuids.length === 0) {
    ...
    try {
      ...
      const evaluatorUuids = selectedEvaluatorUuids;
      ...
      <div className={activeTab === "evaluators" ? undefined : "hidden"}>
        <div
          className={`rounded-lg transition-colors ${
            evaluatorsInvalid ? "bg-red-500/10 border border-red-500 p-4" : ""
          }`}
        >
          {evaluatorsInvalid && (
          ...
          <RunEvaluatorsPanel
            ...
            description="These evaluators evaluate the quality of the speech generated by each model"
          />
        </div>
      </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="w-12 px-4 py-2">
                        {provider.website && (
                          <Tooltip
                            content={`Visit ${provider.label} website`}
                            position="top"
                          >
                            <a
                              ...
                              aria-label={`Visit ${provider.label} website`}
            ...
            {availableDatasets.length === 0 ? (
              <div className="border border-border rounded-xl p-6 flex flex-col items-center justify-center bg-muted/20 text-center">
                ...
                <p className="text-xs text-muted-foreground mb-3">
                  Enter text manually and give it a name to save it as a
                  reusable dataset.
                </p>
                <button
                  onClick={() => setInputMode("inline")}
