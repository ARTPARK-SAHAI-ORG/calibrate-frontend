...
describe("JudgeResultsList", () => {
  ...
  it("shows the picked option's label on a result card, never its description", () => {
    render(
      <JudgeResultsList
        results={[
    ...
    // The description helps an annotator choose while labelling; a result
    // card only says which option was picked.
    expect(screen.getByText("Excellent")).toBeInTheDocument();
    expect(screen.queryByText("Answers fully.")).not.toBeInTheDocument();
    expect(screen.queryByText("Misses the date.")).not.toBeInTheDocument();
  });
...
describe("TestDetailView", () => {
  ...
  it("never shows a reasoning toggle under the agent's answer, even with no judge results", () => {
    // A reasoning string with no per-evaluator judge results has nowhere
    // reliable to render here — the evaluator card next to the conversation
    // is the one place that shows it (or nothing, if there is no card).
    // Repeating it inline under the agent's answer duplicated the tool-call
    // "Tool call test" card, so this pane never draws it, for any test type.
    render(
      <TestDetailView
        history={[]}
        passed={false}
  ...
  it("never shows a reasoning toggle for a tool-call test either", () => {
    render(
      <TestDetailView
        history={[]}
        passed={false}
        output={{ response: "calling a tool with param success true" }}
    ...
    expect(
      screen.queryByText("No tool calls were generated by the LLM"),
    ).not.toBeInTheDocument();
  });
