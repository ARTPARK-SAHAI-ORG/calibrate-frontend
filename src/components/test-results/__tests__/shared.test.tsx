...
describe("TestDetailView", () => {
  ...
  it("hides the reasoning toggle under the agent's answer for a tool-call test", () => {
    // The tool-call verdict card next to the conversation (EvaluationCriteriaPanel's
    // "Tool call test" card) already shows this same reasoning behind its own "See
    // reasoning" toggle. Showing the toggle again here under the agent's answer
  ...
  it("still shows the legacy reasoning toggle for a non-tool-call test with no judge results", async () => {
    const user = setupUser();
    render(
      <TestDetailView
        history={[]}
