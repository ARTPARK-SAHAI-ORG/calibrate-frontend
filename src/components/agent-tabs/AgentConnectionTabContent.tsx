...
type InputFieldType = "text" | "number" | "boolean" | "json";
...
function deriveInputs(rows: InputRow[]): {
...
function seedInputRows(
  defaultInputs: ConnectionConfig["default_inputs"],
): InputRow[] {
  const di = defaultInputs || {};
...
export function AgentConnectionTabContent({
  ...
  const handleAddInput = () => {
    pushInputRows([...inputRows, { key: "", type: "text", value: "" }]);
  };

  const handleRemoveInput = (index: number) => {
    pushInputRows(inputRows.filter((_, i) => i !== index));
  };

  const handleInputRowChange = (index: number, patch: Partial<InputRow>) => {
    pushInputRows(
      inputRows.map((r, i) => {
        if (i !== index) return r;
        const next = { ...r, ...patch };
      ...
      <div className="space-y-6 md:space-y-8">
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Extra fields sent to your agent on every request, alongside
              messages. Set a default value for each. These defaults are used
              when verifying and when running tests.
            </p>
            <div className="space-y-2">
                ...
                <div key={index}>
                  <div className="flex flex-wrap md:flex-nowrap items-center gap-2">
                    <input
                      ...
                      value={row.key}
                      onChange={(e) =>
                        handleInputRowChange(index, { key: e.target.value })
                      }
                      placeholder="Field name"
                      className="flex-1 min-w-0 h-9 md:h-10 px-3 md:px-4 rounded-md text-sm md:text-base border border-border bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent"
                    />
                    <div className="relative">
                      <select
                        value={row.type}
                        onChange={(e) =>
                          handleInputRowChange(index, {
                            type: e.target.value as InputFieldType,
                          })
                        }
                        aria-label="Field type"
                        className="h-9 md:h-10 pl-3 pr-8 rounded-md text-sm md:text-base border border-border bg-background text-foreground focus:outline-none focus:ring-1 focus:ring-accent cursor-pointer appearance-none"
                      >
                    ...
                    {row.type === "boolean" ? (
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          handleInputRowChange(index, { value: e.target.value })
                        }
                        placeholder='{"key": "value"}'
                        aria-label="Field value"
                        className="flex-1 min-w-0 px-3 md:px-4 py-2 rounded-md text-sm md:text-base font-mono border border-border bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent"
                      />
                    ) : (
                      <input
