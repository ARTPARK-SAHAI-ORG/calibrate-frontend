...
- `src/lib/` — utilities (`api.ts`, `sidebar.ts`, `status.ts`, `datasets.ts`, `evaluatorApi.ts`).
  - `evaluatorApi.ts` — the shared `EvaluatorData` type, evaluator error helpers, and the agent↔evaluator association calls: `fetchAgentEvaluators`, `attachEvaluatorToAgent`, `detachEvaluatorFromAgent`, plus `fetchAllEvaluators` / `deleteEvaluator`. Backed by `GET/POST/DELETE /agents/{uuid}/evaluators` (a many-to-many `agent_evaluators` link in the backend, mirroring the annotation-task↔evaluator pattern; attaching allows global default evaluators). The `/evaluators` page and the agent Evaluators tab both consume the same extracted `CreateEvaluatorFlow` (self-contained use-case-picker → sidebar → judge-model → `POST /evaluators` flow) and `DuplicateEvaluatorDialog` from `src/components/evaluators/`.
