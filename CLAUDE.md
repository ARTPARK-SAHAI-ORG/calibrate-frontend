...
**Starting a run or a model comparison goes through one hook on the agent page.** `useAgentRunLaunchers` (`src/components/agent-tabs/useAgentRunLaunchers.tsx`) holds everything the Tests tab used to do inline: `confirmTestRun` (size check, then the "Start the run" confirmation, then the run), `launchTestRun` (the connection check on an unverified connection agent, then `startTestRunOrNotify`), `openCompare` (refuses with a toast on an unverified connection or with benchmarking off, checks the size limit, then opens `BenchmarkDialog`, or the turn-benchmarking-on question first), and `dialogs`, which the caller mounts once. Both the Tests tab and the Evaluations tab (`RunsTabContent`) use it, so `AgentDetail` passes both the same connection and benchmarking settings. That is what lets the ticked rows inside a finished run window or model comparison window be run or compared on their own: `TestRunnerDialog` and `BenchmarkResultsDialog` take `onRunTests` / `onCompareTests`, draw `SelectedTestsStrip` ("N tests selected", Run, Compare) under the search box through the list panels' `selectionStrip` slot, and the tab does the rest. The ticks are the labelling checkboxes, so only labellable test types (response, general, tool call) can be picked this way. A comparison asked for from inside a run window closes that window when the comparison is created, without switching tabs (the comparison window is drawn inside the tab), and the switch to Evaluations happens when the comparison window closes.
