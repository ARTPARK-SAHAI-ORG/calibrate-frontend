...
- Authenticated app pages: `/agents`, `/tools`, `/evaluators`, `/stt`, `/tests`, `/tts`, `/personas`, `/scenarios`, `/simulations`, `/traces`, `/datasets/[id]`, `/workspace-settings`
...
- `src/components/traces/` — the Traces tab (`/traces`, "Production" nav group). Production agent turns ingested via the backend `POST /traces` (API key); this UI is read + curate only (no ingest form). `TracesTable` (desktop table / mobile cards), `TraceDetailDialog` (fetches its own detail via `fetchTrace` so the list holds only `TraceSummary` rows), `TracesEmptyState` (copy-paste ingest snippet → workspace-settings for the key), `ConvertTracesToTestsDialog` (the curation payoff). Backed by `src/lib/tracesApi.ts` + the `useTraces`/`useTraceCount`/`useMaxTraces`/`useTraceDeletion` hooks. Server-side pagination/search/filter (see the paginated-endpoints note). Deep-links: `?conversation_id=` (filter), `?traceId=` (open detail). Deletes go through `POST /traces/bulk-delete` (single, bulk, and a `select_all` "delete all matching" path); there is no per-trace DELETE.
  - **Convert to tests** (`ConvertTracesToTestsDialog` + `convertTracesToTests` in `tracesApi.ts` → backend `POST /traces/convert-to-tests`): a bulk action on the selected traces. Pick `response` (re-run the agent, judge the reply — needs ≥1 llm evaluator, defaulted to the `default-llm-next-reply` fork via `defaultOriginSlug`) or `tool_call` (assert the recorded calls — enabled only when every selected trace has `tool_call_count > 0`, with an "ignore arguments" toggle → `accept_any_arguments`); optionally link the created tests to agents so they're runnable immediately. The dialog is pure (fetches evaluators + agents, calls `convertTracesToTests`, reports via `onConverted`); the page clears the selection (`useBulkDeletion.clearSelection`) and fires a sonner `toast.success` with a "View tests" action → `/tests`. This is the loop close: converted tests then run/benchmark and their run results feed the existing `AddRunToLabellingTaskDialog` labelling flow unchanged.
