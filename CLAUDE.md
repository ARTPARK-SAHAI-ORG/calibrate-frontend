...
- `src/components/human-labelling/` — labelling-task dialogs. `AddRunToLabellingTaskDialog.tsx` powers the "Submit for labelling" action on four result surfaces, each mapping to one task type via `buildItemsFromSource(source)` (the source discriminated union drives the target type via `targetTaskTypeForSource`; you never pick it):
  ...
  - **`test_run` / `benchmark_run` → `llm-tool-call`** (same surfaces): **any** tool-call test is labellable (a human just marks it correct or wrong — no criteria needed; `labellingTaskTypeForRaw` maps `tool_call` → `llm-tool-call`). `targetTaskTypeForSource` is selection-aware for these two sources: a mixed selection targets `llm` (response preferred) and the tool-call tests are skipped; a tool-call-only selection targets `llm-tool-call`. Items map `test_case.history` → `chat_history`, `evaluation.tool_calls` → `expected_tool_calls`, `output.tool_calls` → `actual_tool_calls`, and carry **no evaluators**. The annotator view (`AnnotationJobView`) renders a single **Pass / Fail** control (no evaluators → `EvaluatorsPane`'s empty branch); the verdict rides the `evaluator_id: null` item annotation as `value.value` (boolean), with `value.comment` alongside. `ToolCallItemPane` shows the conversation + actual call + expected spec. (There is no per-item "correct the expected call" editor — labelling is just correct/wrong.)
