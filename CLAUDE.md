...
- `src/components/human-labelling/` — labelling-task dialogs.
  - **Per-item evaluators** (`itemEvaluators.ts`, `ItemEvaluatorPicker.tsx`, `BulkItemEvaluatorsDialog.tsx`): an annotation item may carry its own evaluator list instead of the task's. Every item read returns `evaluator_ids` (what was saved, `null` when the item follows the task, so it only tells you whether the item was narrowed) and `effective_evaluator_ids` (what applies now). **Always render from and write back the effective one** — the saved list can name an evaluator the task has since unlinked, and sending that is a 400. Writes have three cases and they are not interchangeable: key absent means do not touch, explicit `null` means go back to following the task, a list replaces. `useItemEvaluatorSelection` is the one place that decides which of the three a dialog reports; the four item dialogs and the picker all go through it. Removing an evaluator from a task needs no per-item write (the effective list is intersected with the task's on every read); **adding** one skips items that have their own list, which is why `PUT /annotation-tasks/{uuid}/evaluators` returns `customised_item_count` and `ManageEvaluatorsDialog` offers to apply it via `POST /annotation-tasks/{uuid}/items/evaluators`. The labelling job endpoint is the exception: its `items[].evaluator_ids` is already resolved and frozen at job creation, so `AnnotationJobView` renders straight from it.
  - `AddRunToLabellingTaskDialog.tsx` powers the "Submit for labelling" action on four result surfaces, each mapping to one task type via `buildItemsFromSource(source)` (the source discriminated union drives the target type via `targetTaskTypeForSource`; you never pick it):
