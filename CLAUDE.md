...
- `src/hooks/useOrganizations.ts` — `useOrganizations` (list + create + rename), `useActiveOrgUuid` (subscribes to the event), `useOrgMembers` (list + invite + remove), `useWorkspaceApiKeys` (list + create + revoke against `/organizations/{uuid}/api-keys` — the UI labels these "tokens").
...
- `/workspace-settings` — a vertical side-tab layout (`SETTINGS_TABS`: **Admin**, **Tokens**) shown for all workspaces, including personal.
  ...
  - **Tokens tab**: workspace-scoped API tokens for CI / GitHub Actions. Created via `CreateTokenDialog` (two-phase: name form → one-time secret reveal with copy; the backend returns the plaintext token only in the POST response, the list returns a masked `prefix`), revoked via `DeleteConfirmationDialog`. Backend endpoint is `/api-keys`; the UI labels them "tokens" throughout.
