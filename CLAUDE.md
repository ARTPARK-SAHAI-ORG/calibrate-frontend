...
- **Check the diff's coverage before every PR and every push to it.** `codecov/patch` is a blocking check measured against the repo's current coverage (roughly 93% today, and it moves), so a diff at 83% fails even when every test passes. Before opening a PR, and again before each later push to it, list the lines the diff adds or changes and confirm each one is exercised by a test that actually runs in CI:
  - Files under `src/` (except `src/app`) → `npm run test:coverage`, then read the per-file table for the changed files.
  - Files under `src/app` → Jest's `collectCoverageFrom` excludes them, so ONLY the Playwright run reaches them. Cover them with an authenticated spec and check with `npm run test:e2e:integration:coverage`, then read `coverage/e2e/lcov.info` for the file.
  - Error paths inside `catch` blocks count as changed lines. Force the failure in the test (Playwright `page.route` interception) instead of leaving them uncovered.
  - Avoid adding a branch the tests cannot exercise. An `err instanceof Error ? ... : ...` in a page counts as a partial hit and drags the patch below target; call a covered helper (for example `parseBackendErrorMessage` in `src/lib/parseBackendError.ts`) instead.
  - After a push, confirm the real number rather than assuming: `gh pr checks <n>`, or the Codecov API (`https://api.codecov.io/api/v2/github/ARTPARK-SAHAI-ORG/repos/calibrate-frontend/compare/?pullid=<n>`, read `totals.patch`). Keep fixing until it passes; do not hand back a PR with a red patch check.
