...
- Authenticated app pages: `/agents`, `/tools`, `/evaluators`, `/stt`, `/tests`, `/tts`, `/personas`, `/scenarios`, `/simulations`, `/datasets/[id]`, `/workspace-settings`
  - `/tests` and `/tools` have no sidebar entry: they are reachable by their address and from the agent detail tabs. Everything else in this list is in the sidebar, `/evaluators` included (under Scenarios, in the Agent simulations section).
...
- `src/components/evaluations/EvaluatorLibraryPanel.tsx` — every evaluator of one kind, with View, Delete and Create evaluator, drawn as the same cards. This is the **Evaluators tab on `/stt` and `/tts`** (`?tab=evaluators`, next to Evaluations and Datasets), so evaluators can be looked at, deleted and created without starting an evaluation. Nothing is attached here: it is the library filtered to `stt` or `tts`.
