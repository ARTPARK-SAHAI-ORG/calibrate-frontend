...
**Running a model comparison again reopens the picker, filled in.** Rerun on a finished comparison hands its parent a `BenchmarkRerunRequest` (`{ models, testUuids, testNames, parallelModels }`, exported from `BenchmarkResultsDialog`), and the Evaluations tab passes that to `openCompare` as a third `ComparePreset` argument, which `BenchmarkDialog` opens with through `initialModels` / `initialParallelModels`. So the reader can drop a model, add one, or switch how the models run before starting it again. Going through `openCompare` also puts a rerun behind the same checks a comparison started from the Tests tab passes: the connection check, benchmarking being on, and the size limit. It used to re-POST the same comparison straight from the window, which is what `BenchmarkRerunDialog` and its remounting `key` existed for; the Evaluations tab no longer uses either. `initialModels` are model ids, and an id no longer in the provider list still gets a row named after the id rather than disappearing, so a retired model is visible and can be swapped.
