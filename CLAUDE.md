...
- `src/components/traces/` — the Traces tab on the agent detail page (`/agents/[uuid]?tab=traces`, the tab after Tests). There is no `/traces` route and no sidebar entry: **traces belong to one agent**. The customer's backend ingests them with `POST /traces` (API key) carrying the agent's uuid as a top-level `agent_id`; this UI is read + curate only (no ingest form). `TracesTable` (desktop table / mobile cards), `TraceDetailDialog` (fetches its own detail via `fetchTrace` so the list holds only `TraceSummary` rows), `TracesEmptyState` (a three-step setup guide, one step on screen at a time with finished steps collapsed: create a key through the reused `CreateApiKeyDialog` + `useWorkspaceApiKeys`, copy a pre-filled snippet in cURL/Python/JavaScript beside a field list that groups `message_id`/`conversation_id`/`metadata` under **Optional**, then a **Check for traces** button — nothing polls), `ConvertTracesToTestsDialog` (the curation payoff). Backed by `src/lib/tracesApi.ts` + the `useTraces`/`useTraceDeletion` hooks. There is deliberately no stored-traces usage line: an agent-scoped count beside a workspace-wide cap reads as room to spare when the workspace is full. Every read is agent-scoped and server-paginated/searched/filtered (see the paginated-endpoints note). Deep-links: `?conversation_id=` (filter), `?traceId=` (open detail), on top of the tab's `?tab=traces`. Deletes go through `POST /traces/bulk-delete` (single, bulk, and a `select_all` "delete all matching" path that carries `agent_id`); there is no per-trace DELETE.
  - **Convert to tests** (`ConvertTracesToTestsDialog` + `convertTracesToTests` in `tracesApi.ts` → backend `POST /traces/convert-to-tests`): a bulk action on the selected traces. Pick `response` (re-run the agent, judge the reply — needs ≥1 llm evaluator, defaulted to the `default-llm-next-reply` fork via `defaultOriginSlug`) or `tool_call` (assert the recorded calls — enabled only when every selected trace has `tool_call_count > 0`, with an "ignore arguments" toggle → `accept_any_arguments`). The dialog does NOT ask which agents to link: it takes `agentUuid` and always sends `agent_uuids: [agentUuid]`, so created tests belong to the agent whose tab this is and are runnable immediately. The dialog is pure (fetches evaluators, calls `convertTracesToTests`, reports via `onConverted`); the tab clears the selection (`useBulkDeletion.clearSelection`) and fires a sonner `toast.success` linking to the created tests. This is the loop close: converted tests then run/benchmark and their run results feed the existing `AddRunToLabellingTaskDialog` labelling flow unchanged.
