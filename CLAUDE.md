...
- Public: `/` (landing), `/login`, `/signup`, `/changelog`, `/learn`, `/public/...` (shareable result pages), `/invite/[token]` (join a workspace from a link)
...
- Authenticated app pages: `/agents`, `/tools`, `/stt`, `/tts`, `/personas`, `/scenarios`, `/simulations`, `/datasets/[id]`, `/workspace-settings`
  - `/tools` has no sidebar entry: it is reachable by its address and from the agent detail tabs. Everything else in this list is in the sidebar. (`/tests` and `/evaluators` still answer, but nothing links to them and nobody uses them; do not build for them.) The sidebar has two **Evaluators** entries, both on the same `EvaluatorLibraryPanel` the Speech-to-Text and Text-to-Speech pages use: `/agent-evaluators` under Agents (the next-reply and output judges that can be added to an agent) and `/simulation-evaluators` under Scenarios (the conversation ones).
...
- `/workspace-settings` — a vertical side-tab layout (`SETTINGS_TABS`: **Admin**, **API keys**) shown for all workspaces, including personal.
  - **Admin tab**: rename the active workspace, share the invite link, and manage members. Each member row shows the role (`owner` or `admin`) as a pill; the "Remove" button is hidden entirely for `role === "owner"`. On the current user's own row the button reads "Leave" and the confirm dialog uses leave-flavoured copy; after a successful self-leave, `clearActiveOrgUuid()` runs and the user is routed to `/agents` so the bootstrapper picks a fresh workspace.
    - **The invite link** (`InviteLinkPanel` in `src/components/workspace/`, `useOrgInviteLink` in `src/hooks/useOrganizations.ts`, helpers in `src/lib/invites.ts`): a workspace hands out **one** link at a time, the way Discord does it, and it never expires on its own. `GET`/`POST /organizations/{uuid}/invite-link` read it and make it; any member can do both. The backend also has a `DELETE` that turns the link off, and the frontend deliberately does not use it: turning a link off, and replacing one, both stop an address other people are already holding from working, and neither is worth carrying until somebody asks for it. A `404` from the `GET` is the ordinary "nobody has made a link yet" state, not a failure, and the hook turns it into `inviteLink: null` with no error. Opening the link lands on the public `/invite/[token]` page, which reads the workspace name from `GET /public/invite/{token}` without signing in (so someone with no account can see what they were invited to) and joins with `POST /invites/{token}/accept`, which is idempotent and returns the workspace. Joining adds the caller as `admin`, the same role the add-by-email form gives. `api.ts` strips `X-Org-UUID` from `/invites` requests for the same reason it strips it from `/organizations`: the invite page carries no workspace in its address, so the header would otherwise carry over whichever workspace the reader last had open.
...
- `src/components/agent-tabs/` — the tabbed UI on `/agents/[uuid]` (Agent / Tools / Evaluations / Tests / Evaluators / Traces / Settings for build agents; Evaluations / Tests / Evaluators / Traces / Connection / Settings for connection agents). A build agent opens on Agent; a connection agent opens on Evaluations, with Connection sitting next to Settings because it is set up once. Which tabs appear is data-driven via `calibrateTabs` / `connectionTabs` arrays and a `tabLabels` map.
  - The **Evaluators tab** (`EvaluatorsTabContent`) lets users curate which evaluators matter for an agent: attach existing library evaluators (`AddEvaluatorsDialog`, a searchable checkbox picker; rows show the name and description only, with the simulation-only evaluators left out; clicking a row's name opens how that evaluator judges — model, prompt, values, scores — in a second column via `EvaluatorPromptPreview`, which reads `GET /evaluators/{uuid}` since the list endpoints carry no prompt text. Ticking is the checkbox only. Shared by every dialog using `EvaluatorPicker`), create new ones inline (`CreateEvaluatorFlow`, which auto-attaches on create), duplicate, detach (kept in library), or permanently delete (owned only). Persistence is a real agent↔evaluator association — see the endpoints note below.
  - The **Tests tab** is wired to that same agent evaluator list: a NEW test seeds its evaluators from the agent's connected evaluators (`llm` type), falling back to the `default-llm-next-reply` correctness evaluator. After a create/update that references evaluators not yet on the agent, `TestsTabContent` prompts to add them to the agent's defaults (attach only — removing an evaluator from a test never detaches it from the agent). `AddTestDialog` takes an `agentEvaluatorUuids` prop for the seeding. A test written from scratch also opens with a worked example already in its boxes — name, conversation (or the single input), and the criteria of that fallback correctness evaluator — from `sampleTest` in `src/lib/testSamples.ts`, the same example the type picker shows. Editing, duplicating, labelling items, and any caller passing `allowAgentLastMessage` / `requireAssistantLastMessage` keep their own content. An evaluator the reader connected themselves is never filled in, and neither is the tool a tool call test expects.

**The agent page has a separate Evaluations tab.** `RunsTabContent` (labelled **Evaluations**, tab id still `runs` so `?tab=runs` links keep working; after Tests, both agent kinds) is the only place an agent's past runs are listed: one table with **Run / Result / Tests / Models / Created at**, no test-versus-benchmark split and no type label. (A column naming the evaluators that judged the run is waiting on the list endpoint to carry their names.) It is server-paginated through `useAgentRuns` (`limit`/`offset` on `GET /agent-tests/agent/{uuid}/runs`, page size from the shared `usePageSize`) and the result filter is server-side too (`has_failures` for all-passed / all-failed, `status=failed` for error). Clicking a row opens `TestRunnerDialog` for a plain run or `BenchmarkResultsDialog` for a multi-model one, and `?runId=` deep-links the plain-run window. Each row also carries a delete icon: `DELETE /agent-tests/job/{task_id}` takes the run and its results away, one at a time (the backend has no bulk delete for runs), and only once the run has finished, so a run that is queued or still going has the icon greyed out with a tooltip saying why. `TestsTabContent` no longer holds a runs list: it starts runs, opens the run window, and calls `onRunStarted` so `AgentDetail` remounts the Runs tab to pick the new run up. Both tabs name their open run in the address as `?runId=`, and `AgentDetail` passes each an `isActive` flag so only the tab on screen reads that param, which is what stops one run opening a window on both.

**Test runs are created by the parent, never by the dialog.** `src/lib/testRunApi.ts` owns the two calls (`startTestRun` to create, `fetchTestRun` to read). Every "run" action on the agent Tests tab calls `startTestRun` FIRST, prepends an optimistic row to the runs list, and only then opens `TestRunnerDialog` with the returned `taskId`. The dialog takes `{ isOpen, onClose, agentUuid, agentName, taskId, onNewRun }` and is fetch/poll/render only: it holds one `run` response as its sole source of run content and derives every row, count, and aggregate from it, so there is no local copy to reconcile or reset. Rerun works the same way — the dialog calls `startTestRun` and reports the new id through `onNewRun`, which is what puts rerun-created runs in the list. Pass `null` for `testUuids` to run every test linked to the agent, and pass `testCount` with it (the list itself does not say how big the run is). Because the dialog needs a real run id, each tab renders it ONCE, driven by a run id, rather than keeping separate "new run" and "view past run" instances.

**Starting a run or a model comparison on the agent page goes through `useAgentRunLaunchers`** (`src/components/agent-tabs/useAgentRunLaunchers.tsx`): the size check, the "Start the run" confirmation, the connection check on an unverified connection agent, and the model picker with its turn-benchmarking-on question. The Tests tab and the Evaluations tab both use it, with the same `AgentRunLauncherSettings` from `AgentDetail`. That is what lets the ticked rows inside a finished run window or comparison window be run or compared on their own: `TestRunnerDialog` and `BenchmarkResultsDialog` take `onRunTests` / `onCompareTests` and draw `SelectedTestsStrip` under the search box. A comparison asked for from inside a run window closes that window when the comparison is created, without switching tabs (the comparison window is drawn inside the tab); the switch to Evaluations happens when the comparison window closes.

**A run is read light, and one test is read in full when someone opens it.** `GET /agent-tests/run/{task_id}?mode=summary` and `GET /agent-tests/benchmark/{task_id}?mode=summary` leave `test_case`, `output`, `judge_results` and `inputs` off every case, keeping its name, verdict, reason, latency, cost and a `test_type` (`response` / `general` / `tool_call` / `conversation`). A run of nearly two thousand tests goes from 4.64 MB to 0.86 MB. Everything above the case rows is unchanged, including the `evaluators[]` rubrics, and a run-level `evaluator_summary` carries the totals per evaluator so nothing counts verdicts in the browser any more. One case comes from `GET /agent-tests/run/{task_id}/results/{test_uuid}`, one path for both kinds of run, with `?model=` for a comparison because a model name contains a slash. Which test a row ran is `rowTestUuid`, never `test_case_id` read straight: calibrate replaces the id it is sent with the test's name, so on some runs `test_case_id` holds a name, and `test_uuid` is the test's real id. The shared links carry the same pair under their token. `fetchTestRun(..., "summary")` and `fetchTestCase` in `src/lib/testRunApi.ts` are the only ways to ask; `runEvaluatorSummary` and `rowTestType` / `isToolCallRow` in `src/lib/testRunSummary.ts` are the only ways to read the two new fields. `pass_rate` is out of 100 on both a run's totals and a model's, so nothing rescales it. Export and Submit for labelling need every case's conversation and reply, so they read the run in full at the moment they are clicked, never on open. A test file that mocks a run URL must match it with the query attached: adding `?mode=summary` broke thirty-one tests that matched the address exactly.

**A finished run is remembered for the session, and a run list never downloads a whole run.** `fetchTestRun` keeps the last three runs the backend has finished with (`getCachedTestRun` reads one back, `clearTestRunCache` empties it, and a rename drops that run's copy). Reopening the window shows the remembered copy at once and still fetches, so a name or a share link changed elsewhere still lands. The cap is small on purpose: one run of nearly two thousand tests is several megabytes. Any test that reuses a task id with different runs behind it must call `clearTestRunCache` between cases. The runs list (`useAgentRuns`) keeps an unfinished run up to date by asking for the list again, quietly, while any run on the page is still going. They used to fetch the full result of every unfinished run every three seconds to read six counters the list already carries. Do not put that back.

**A run can be renamed, from the pencil beside the name at the top of its window.** `EditableRunName` (`src/components/EditableRunName.tsx`) is the one place that does it: the pencil opens the shared `RenameDialog`, with `allowEmpty` so the box can be cleared (a run has an automatic name to fall back on, unlike a dataset). It is shared by `TestRunnerDialog` and `BenchmarkResultsDialog`, and it calls `renameRun` in `src/lib/testRunApi.ts` (`PATCH /agent-tests/run/{task_id}/name`, which takes either kind of run). Emptying the box sends `null`, and the backend answers with the automatic name it goes back to ("Run 3", "Benchmark 2"), which is what goes on screen. Both dialogs report the new name through `onRenamed` so the list behind the window reads the same. The name itself now comes from the run's own detail endpoint, and from `GET /public/test-run/{token}` / `GET /public/benchmark/{token}` on the shared links, so nothing carries it over from the list. What a reader sees is always `runDisplayName` / `modelComparisonName` in `src/lib/testTypes.ts`.

**A test that produced no answer is not a wrong answer.** A run has two ways to fail a test: the agent answered and the judge said no, or the agent never answered at all (a timeout, an HTTP error, an unreachable judge). The backend marks the second kind with `unanswered` on the test row, and counts them per run as `unanswered_tests` (plus `stopped_early` when the run gave up before starting every test; the run detail, the comparison detail and the runs list all carry the flag). `isUnanswered` in `src/lib/testTypes.ts` is the ONE rule, used by the runs list, the run window and the model comparison panel alike — never re-derive it. In particular `passed === null` means only "not finished yet": before calibrate 0.0.74 an unanswered test came back with no verdict, and it now comes back as `passed: false`, so reading a missing or false verdict as "never answered" is the exact bug this flag exists to correct. Such a test is kept out of the pass rate, shown as "could not be run" with the backend's reason (which arrives in `reasoning`), and counted separately in the run summary and the runs list.

**A run that broke is shown with what it did finish, and with what the backend actually recorded.** `error` on a run or comparison detail is raw text the backend has (calibrate's own ❌ line, or how the process ended, such as "calibrate-agent process killed by signal 15 (SIGTERM: Terminated)"), or null; older runs carry true or false. `runErrorText` in `src/lib/testRunApi.ts` reads it. The words a reader sees come from the app, in `RunFailureBox` (`src/components/RunFailureBox.tsx`): "Something went wrong", one sentence, and the raw text in a Details block with a Copy button. With no finished tests the box replaces the window ("The evaluation run failed before it produced any result."). With finished tests the window reads as usual: the rows stay, Rerun stays, the Results tab opens with the same red box saying "The evaluation failed after N of M tests." (the comparison, through `runFailureReason` on `BenchmarkResultView` and `failureReason` on `BenchmarkCombinedLeaderboard`, which also builds the leaderboard rows from the finished tests when the backend sent no summary), and the tests never reached are drawn as not run: the backend marks them `not_run` the way a stopped run does, and `BenchmarkOutputsPanel` takes `runFailed` so a row with no verdict reads the same once the run is over. A plain run shows the same box above its summary, through `failureDetails` on `TestRunSummary`, and both windows open such a run on the summary, where the box is. The shared links accept a failed run too. The mark beside a run's name (`RunStateMark`, one rule in `runStateOf`) has four states: finished, `gave_up` (amber, `stopped_early`, "Partially complete as some tests could not be run"), stopped, and error, and the list and the windows all read the same rule.

- `src/components/traces/` — the Traces tab on the agent detail page (`/agents/[uuid]?tab=traces`, the tab after Tests). There is no `/traces` route and no sidebar entry: **traces belong to one agent**. The customer's app ingests them with `POST /traces` (API key) carrying the agent's uuid as a top-level `agent_id`; `message_id` and `conversation_id` are optional and nullable, and the ingest response has no `created` field. This UI is read + curate only (no ingest form). **Trace scoring.** The switch **Score new traces automatically** sits in a Traces section on the agent Settings tab and is saved at once through `PUT /agents/{uuid}` with `auto_score_traces` (on by default once the backend change lands). Which linked evaluators can score comes from `GET /agents/{uuid}/trace-scoring-eligibility`; turning the switch on is refused with a 422 when none can. `useAgentTraceScoring` is called once in `AgentDetail` and handed to both the Settings tab and the Traces tab as `TraceScoringControls`. Once traces exist, the Traces tab shows one line saying scoring is on with a **Turn off** button (or that it is off, with a Turn on in Settings button) and a second line naming the evaluators that cannot score traces and why. `TracesTable` is a desktop table / mobile card list with **Input**, **Output** (reply, else the tool calls as chips), one column per evaluator that can score (from the eligible list, read from each row's `scores`; a spinner spans those columns while the trace is waiting or being scored, and a failed or skipped run is a pill), and **Created**, plus selection and delete controls. While any row on the page is still being scored, `useTraces` asks for the page again every 3 seconds, one list request rather than one per row. Pagination uses the shared **server-paginated list bar** via `ServerPaginatedListBar` (see Conventions) directly above the table. `TraceDetailDialog` fetches its own detail via `fetchTrace` and reuses `TestDetailView` so a trace reads like a test run; it has three columns: metadata, the input and output, then one verdict card per evaluator from the latest run only, read from `GET /traces/{uuid}/scores`. `TracesEmptyState` is the three-step setup guide (create OR paste-and-check a key via `validateApiKeyForAgent`, send a trace, check it arrived); the request itself lives in `TraceIngestSnippet` so `TraceIngestCodeDialog` can show the same thing from the toolbar's **Integration guide** after the first trace lands, when the setup steps are gone (with `YOUR_API_KEY`, since a key's text exists only at creation). Bulk actions on a selection: **Add to tests** (`ConvertTracesToTestsDialog`) and **Submit for labelling** (`TraceLabellingEvaluatorsDialog` to pick evaluators, then the shared `AddRunToLabellingTaskDialog` with its `traces` source; the tab fetches each selected trace in full first, because the list holds only previews and that dialog wants a ready source). Both dialogs pick evaluators through `useAgentLlmEvaluators` + the shared `EvaluatorPicker`: llm evaluators minus any that declare `live_version.variables` (there is nowhere to ask for their values, and neither request carries them), preselected to this agent's own, else the `default-llm-next-reply` fork. Backed by `src/lib/tracesApi.ts` + the `useTraces`/`useTraceDeletion`/`useAgentTraceScoring` hooks. Every list read is agent-scoped, server-paginated (page size from `usePageSize`, shared with the labelling items tab under one saved setting; the backend caps a page at `MAX_TRACES_PAGE_SIZE`). It is the ONE server-paginated list in the app; do not "harmonize" it to the fetch-everything pattern. **Search is server-side too**: the toolbar's search box is debounced (300ms) into `useTraces`'s `q`, which resets to the first page; while anything is typed an empty result shows "No traces match your search." rather than the setup steps. Deep-links are `?tab=traces` and `?traceId=`. Deletes go through `POST /traces/bulk-delete`, which takes `{ trace_ids }` ONLY — no `select_all`, no filters, unknown fields rejected — so there is no "delete everything matching" path and no per-trace DELETE.
...
- `src/components/eval-details/` — shared display components for STT / TTS / simulation-run result pages (metrics grids, provider cards, tables).
  ...
  - A model comparison is drawn by ONE component on both surfaces: `BenchmarkResultView` (`src/components/eval-details/BenchmarkResultView.tsx`), rendered by `BenchmarkResultsDialog` (the signed-in run window, `surface="window"`) and by `/public/benchmark/[token]` (the shared link, `surface="public"`). It owns the tab list, which tab draws which panel and with what props, reading one test in full when someone opens it, and which model is open. Each surface passes its own `fetchCase`, because the addresses differ (`/agent-tests/run/{task_id}/results/{test_uuid}?model=` when signed in, `/public/benchmark/{token}/results/{test_uuid}?model=` on the link). Everything only the run window has stays there and arrives as an opt-in prop, never as a second copy of the markup: the name and its pencil, rerun, stop, sharing, Submit for labelling, and the state while the run is still going. `withTestType`, `benchmarkCsvRows` and `evaluatorsByUuid` are exported from the same file so neither surface writes its own. Model names are left to the panels, which name them through `displayModelName` in `src/lib/modelName.ts`. Before this, both files wrote the same layout and drifted: the shared link lost the priority sliders (#537), downloaded a results file with no conversation or reply in it, and did not say what kind of test each row ran.
...
**What an agent does** is a second, separate field: `interaction_type` is `"conversation"` (it talks with a user) or `"general"` (Single Agent Response, one input in and one output out). `INTERACTION_TYPES` in `src/components/ui/InteractionTypePill.tsx` holds the label and description shown everywhere, `InteractionTypePill` shows it on the agents list and the agent page, and `InteractionTypeChooser` is the two-card picker shared by the new-agent dialog and `DuplicateAgentDialog`. The backend sets it when the agent is created and `AgentUpdate` cannot change it, so copying an agent, which sends `interaction_type` alongside the new name to `POST /agents/{uuid}/duplicate` (optional there, and a value outside the two kinds is refused), is the only way to move an existing agent between the two. A copy carries the agent's tools and evaluators and never its tests, and when the kind changes the backend leaves behind the evaluators that cannot judge the new kind, so a conversation agent copied as a single response one can arrive with none. `DuplicateAgentDialog` says both of those before the copy is made.
...
- **Test runs** — `startTestRunOrNotify` in `src/lib/testRunApi.ts`, counted in tests. Covers the agent Tests tab and Rerun inside `TestRunnerDialog`.
