...
- Authenticated app pages: `/agents`, `/tools`, `/evaluators`, `/stt`, `/tests`, `/tts`, `/personas`, `/scenarios`, `/simulations`, `/datasets/[id]`, `/workspace-settings`
  - `/tests`, `/tools` and `/evaluators` have no sidebar entry: they are reachable by their address and from the agent detail tabs. Everything else in this list is in the sidebar. `/simulation-evaluators` is the sidebar's **Evaluators** entry, under Scenarios: the conversation evaluators on the same `EvaluatorLibraryPanel` the Speech-to-Text and Text-to-Speech pages use, not the whole `/evaluators` library.
...
- `src/components/evaluations/EvaluatorLibraryPanel.tsx` — every evaluator of one kind, with View, Delete and Create evaluator, drawn as the same cards. This is the **Evaluators tab on `/stt` and `/tts`** (`?tab=evaluators`, next to Evaluations and Datasets) and the whole of **`/simulation-evaluators`** (the sidebar's Evaluators entry, `conversation`), so evaluators can be looked at, deleted and created without starting a run. Nothing is attached here: it is the library filtered to one kind.
